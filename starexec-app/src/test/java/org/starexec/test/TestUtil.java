...
public class TestUtil {
	...
	/**
	 * produces a random name of the maximum length for a new space. Useful for
	 * testing
	 * 
	 * @return
	 */
	...
	/**
	 * Creates a fake solver object with randomized parameters. The solver will not
	 * be in the database
	 * 
	...
	public static HttpServletRequest getMockHttpRequest(int userId) {
		...
		Mockito.when(session.getAttribute(SessionUtil.USER)).thenReturn(mockUser);
		return request;
	}

	@SuppressWarnings("rawtypes")
	public static HttpServletRequest getMockHttpRequest(int userId, Map params) {
		HttpServletRequest request = getMockHttpRequest(userId);
		if (params != null) {
			for (Object keyObj : params.keySet()) {
				String key = (String) keyObj;
				Object val = params.get(key);
				if (val instanceof String) {
					Mockito.when(request.getParameter(key)).thenReturn((String) val);
				}
			}
		}
		return request;
	...
	public static HttpServletRequest getMockHttpRequest(int userId, Map params, Map listParams) {
		HttpServletRequest request = getMockHttpRequest(userId, params);
		if (listParams != null) {
			for (Object keyObj : listParams.keySet()) {
				String key = (String) keyObj;
				Object val = listParams.get(key);
				if (val instanceof List) {
					List<String> list = (List<String>) val;
					String[] array = list.toArray(new String[0]);
					Mockito.when(request.getParameterValues(key)).thenReturn(array);
				}
			}
		}
		return request;
