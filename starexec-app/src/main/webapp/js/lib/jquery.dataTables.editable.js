...
(function($) {
	...
	$.fn.makeEditable = function(options) {
		...
		function _fnOnRowAdded(data) {
			...
			if (properties.fnOnNewRowPosted(data)) {
				...
				$("input:text[rel],input:radio[rel][checked],input:hidden[rel],select[rel],textarea[rel],span.datafield[rel]",
					oAddNewRowForm).each(function() {
					var rel = $(this).attr("rel");
					var sCellValue = "";
					if (rel >= iColumnCount) {
						properties.fnShowError("In the add form is placed input element with the name '" + $(
							this)
						.attr("name") + "' with the 'rel' attribute that must be less than a column count - " + iColumnCount,
							"add");
					} else {
						...
						sCellValue = sCellValue.replace(properties.sIDToken,
							data);
		...
		function _fnOnRowDelete(event) {
			...
			var id = fnGetCellID($('tr.' + properties.sSelectedRowClass + ' td',
				oTable)[0]);
		...
		*/
		...
		function fnOnEdited(
			result, sOldValue, sNewValue, iRowIndex, iColumnIndex,
			iRealColumnIndex) {
		...
		return this.each(function() {
			...
			$("tbody", oTable).click(function(event) {
				if ($(event.target.parentNode)
					.hasClass(properties.sSelectedRowClass)) {
					$(event.target.parentNode)
					.removeClass(properties.sSelectedRowClass);
					if (oDeleteRowButton != null) {
						_fnDisableDeleteButton();
					}
				} else {
					...
					$(event.target.parentNode)
					.addClass(properties.sSelectedRowClass);
					if (oDeleteRowButton != null) {
