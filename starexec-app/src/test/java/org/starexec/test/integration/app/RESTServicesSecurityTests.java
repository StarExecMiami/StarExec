 ...
 * security reasons. These
 * tests exist to make sure that requests that would violate permissions
 * actually do get rejected. Tests
...
public class RESTServicesSecurityTests extends TestSequence {
	...
	private void getSolverComparisonsPaginatedTest() {
		...
		assertResultIsInvalid(
				services.getSolverComparisonsPaginated(false, -1, cId, cId, TestUtil.getMockHttpRequest(user.getId())));
	}

	@StarexecTest
	private void getAnonymousLinkForPrimitiveTest() {
		assertResultIsInvalid(services.getAnonymousLinkForPrimitive("job", job.getId(), "none",
				TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(
				services.getAnonymousLinkForPrimitive("job", -1, "none", TestUtil.getMockHttpRequest(admin.getId())));
		assertResultIsInvalid(services.getAnonymousLinkForPrimitive("invalid", job.getId(), "none",
	...
	private void getSpaceOverviewGraphTest() {
		...
		assertResultIsInvalid(
				services.getSpaceOverviewGraph(1, job.getPrimarySpace(), TestUtil.getMockHttpRequest(user.getId())));
	}
	...
	private void getJobStatsTest() {
		assertResultIsInvalid(
				services.getJobStatsPaginated(1, -1, false, false, false, TestUtil.getMockHttpRequest(admin.getId())));
		assertResultIsInvalid(services.getJobStatsPaginated(1, job.getPrimarySpace(), false, false, false,
				TestUtil.getMockHttpRequest(user.getId())));
	}
	...
	private void getJobPairsInSpaceHierarchyByConfigPaginatedTest() {
		int cId = job.getJobPairs().get(0).getPrimaryConfiguration().getId();
		assertResultIsInvalid(services.getJobPairsInSpaceHierarchyByConfigPaginated(1, false, job.getPrimarySpace(),
				"all", cId, TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(services.getJobPairsInSpaceHierarchyByConfigPaginated(1, false, -1, "all", cId,
				TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(services.getJobPairsInSpaceHierarchyByConfigPaginated(1, false, job.getPrimarySpace(),
				"badtypestring", cId, TestUtil.getMockHttpRequest(user.getId())));
	}
	...
	private void postProcessJobTest() {
		assertResultIsInvalid(services.postProcessJob(job.getId(), 1, postProcessor.getId(),
				TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(services.postProcessJob(job.getId(), 1, -1, TestUtil.getMockHttpRequest(admin.getId())));
		assertResultIsInvalid(
				services.postProcessJob(-1, 1, postProcessor.getId(), TestUtil.getMockHttpRequest(admin.getId())));
	}
	...
	private void rerunSinglePairTest() {
		assertResultIsInvalid(
				services.rerunJobPair(job.getJobPairs().get(0).getId(), TestUtil.getMockHttpRequest(user.getId())));
	...
	private void rerunAllPairsOfStatusTest() {
		assertResultIsInvalid(services.rerunJobPairs(job.getId(), StatusCode.STATUS_KILLED.getVal(),
				TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(services.rerunJobPairs(-1, StatusCode.STATUS_KILLED.getVal(),
				TestUtil.getMockHttpRequest(admin.getId())));
	}
	...
	private void editJobNameTest() {
		assertResultIsInvalid(
				services.editJobName(job.getId(), "test name", TestUtil.getMockHttpRequest(user.getId())));
	...
	private void editJobDescTest() {
		assertResultIsInvalid(
				services.editJobDescription(job.getId(), "test desc", TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getAllBenchInSpaceTest() {
		assertResultIsInvalid(
				services.getAllBenchmarksInSpace(space.getId(), TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getPrimitiveDetailsPaginatedTest() {
		assertResultIsInvalid(services.getPrimitiveDetailsPaginated(space.getId(), "solver",
				TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(
				services.getPrimitiveDetailsPaginated(-1, "solver", TestUtil.getMockHttpRequest(admin.getId())));
	}

	@StarexecTest
	private void getSolverWebsitesTest() {
		assertResultIsInvalid(
				services.getWebsites("solver", solver.getId(), TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(services.getWebsites("solver", -1, TestUtil.getMockHttpRequest(admin.getId())));
		assertResultIsInvalid(
				services.getWebsites("invalid", solver.getId(), TestUtil.getMockHttpRequest(admin.getId())));
	}
	...
	private void addSolverWebsiteTest() {
		assertResultIsInvalid(services.addWebsite("solver", solver.getId(),
				TestUtil.getMockHttpRequest(user.getId(), getNameAndUrlParams())));
		assertResultIsInvalid(
				services.addWebsite("solver", -1, TestUtil.getMockHttpRequest(admin.getId(), getNameAndUrlParams())));
		assertResultIsInvalid(services.addWebsite("invalid", solver.getId(),
				TestUtil.getMockHttpRequest(admin.getId(), getNameAndUrlParams())));
	}

	@StarexecTest
	private void addSpaceWebsiteTest() {
		assertResultIsInvalid(services.addWebsite("space", space.getId(),
				TestUtil.getMockHttpRequest(user.getId(), getNameAndUrlParams())));
		assertResultIsInvalid(
				services.addWebsite("space", -1, TestUtil.getMockHttpRequest(admin.getId(), getNameAndUrlParams())));
	}

	@StarexecTest
	private void addUserWebsiteTest() {
		assertResultIsInvalid(services.addWebsite("user", admin.getId(),
				TestUtil.getMockHttpRequest(user.getId(), getNameAndUrlParams())));
		assertResultIsInvalid(
				services.addWebsite("user", -1, TestUtil.getMockHttpRequest(admin.getId(), getNameAndUrlParams())));
	}
	...
	private void addUsersToSpaceTest() {
		...
		assertResultIsInvalid(services.addUsersToSpace(space.getId(),
				TestUtil.getMockHttpRequest(user.getId(), params, getSelectedIdParams(user.getId()))));
		assertResultIsInvalid(services.addUsersToSpace(-1,
				TestUtil.getMockHttpRequest(admin.getId(), params, getSelectedIdParams(user.getId()))));
		assertResultIsInvalid(services.addUsersToSpace(space.getId(),
				TestUtil.getMockHttpRequest(admin.getId(), new HashMap<>(), getSelectedIdParams(user.getId()))));
		assertResultIsInvalid(
				services.addUsersToSpace(space.getId(), TestUtil.getMockHttpRequest(admin.getId(), params)));
	}
	...
	private void copySolversToSpaceTest() {
		...
		assertResultIsInvalid(services.copySolversToSpace(space.getId(),
				TestUtil.getMockHttpRequest(admin.getId(), new HashMap<>(), getSelectedIdParams(solver.getId())),
				response));
	}
	...
	private void copyBenchmarksToSpaceTest() {
		...
		assertResultIsInvalid(services.copyBenchToSpace(-1,
				TestUtil.getMockHttpRequest(admin.getId(), params, getSelectedIdParams(benchmarkIds.get(0))),
				response));
		assertResultIsInvalid(services.copyBenchToSpace(space.getId(),
				TestUtil.getMockHttpRequest(admin.getId(), params), response));
		assertResultIsInvalid(services.copyBenchToSpace(space.getId(),
				TestUtil.getMockHttpRequest(admin.getId(), params, getSelectedIdParams(-1)), response));
		assertResultIsInvalid(services.copyBenchToSpace(space.getId(),
				TestUtil.getMockHttpRequest(admin.getId(), new HashMap<>(), getSelectedIdParams(benchmarkIds.get(0))),
				response));
	}

	@StarexecTest
	private void getBenchmarkUploadDescription() {
		assertResultIsInvalid(services.getBenchmarkUploadDescription(benchmarkStatus.getId(),
				TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getJobPairStdoutTest() {
		try {
			services.getJobPairStdout(job.getJobPairs().get(0).getId(), 1, 100,
					TestUtil.getMockHttpRequest(user.getId()));
	...
	private void setSettingsProfileForUserTest() {
		assertResultIsInvalid(services.setSettingsProfileForUser(Communities.getDefaultSettings(space.getId()).getId(),
				user.getId(), TestUtil.getMockHttpRequest(user.getId())));
	}

	@StarexecTest
	private void deleteDefaultSettingsTest() {
		assertResultIsInvalid(services.deleteDefaultSettings(Communities.getDefaultSettings(space.getId()).getId(),
				TestUtil.getMockHttpRequest(user.getId())));
	}
	...
	private void editCommunityDefaultSettingsTest() {
		...
		assertResultIsInvalid(services.editCommunityDefaultSettings("defaultsolver",
				Communities.getDefaultSettings(space.getId()).getId(),
				TestUtil.getMockHttpRequest(user.getId(), params)));
		assertResultIsInvalid(
				services.editCommunityDefaultSettings("fakeattr", Communities.getDefaultSettings(space.getId()).getId(),
						TestUtil.getMockHttpRequest(admin.getId(), params)));
	}
	...
	private void editCommunityTest() {
		...
		params.put("val", solver.getId() + "");
		assertResultIsInvalid(
				services.editCommunityDetails("name", Communities.getDefaultSettings(space.getId()).getId(),
						TestUtil.getMockHttpRequest(user.getId(), params)));
		assertResultIsInvalid(
				services.editCommunityDetails("fakeattr", Communities.getDefaultSettings(space.getId()).getId(),
						TestUtil.getMockHttpRequest(admin.getId(), params)));
	}
	...
	private void editSpaceTest() {
		...
		assertResultIsInvalid(services.editSpace(Communities.getDefaultSettings(space.getId()).getId(),
				TestUtil.getMockHttpRequest(user.getId(), params)));
		assertResultIsInvalid(services.editSpace(Communities.getDefaultSettings(space.getId()).getId(),
	...
	private void editQueueInfoTest() {
		...
		assertResultIsInvalid(
				services.editQueueInfo(Queues.getAllQ().getId(), TestUtil.getMockHttpRequest(user.getId(), params)));
	}
	...
	private void editQueueInfoInvalidParamsTest() {
		...
		params.put("wallTimeout", "10");
		assertResultIsInvalid(
				services.editQueueInfo(Queues.getAllQ().getId(), TestUtil.getMockHttpRequest(admin.getId(), params)));
	}
	 ...
	 * assertResultIsInvalid(services.runAllTests(TestUtil.getMockHttpRequest(user.
	 * getId())));
	 * }
	 */
	...
	private void recycleOrphanedBenchmarksTest() {
		assertResultIsInvalid(
				services.recycleOrphanedBenchmarks(admin.getId(), TestUtil.getMockHttpRequest(user.getId())));
	...
	private void editBenchmarkTest() {
		...
		assertResultIsInvalid(
				services.editBenchmarkDetails(solver.getId(), TestUtil.getMockHttpRequest(user.getId(), params)));
		assertResultIsInvalid(
				services.editBenchmarkDetails(solver.getId(), TestUtil.getMockHttpRequest(admin.getId())));
	...
	private void demoteLeaderTest() {
		assertResultIsInvalid(
				services.demoteLeader(space.getId(), user.getId(), TestUtil.getMockHttpRequest(user.getId())));
	...
	private void editUserPasswordTest() {
		...
		assertResultIsInvalid(
				services.editUserPassword(admin.getId(), TestUtil.getMockHttpRequest(user.getId(), params)));
	...
	private void editUserPermissionsTest() {
		assertResultIsInvalid(
				services.editUserPermissions(space.getId(), admin.getId(), TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(
				services.editUserPermissions(-1, admin.getId(), TestUtil.getMockHttpRequest(admin.getId())));
		assertResultIsInvalid(
				services.editUserPermissions(space.getId(), -1, TestUtil.getMockHttpRequest(admin.getId())));
	...
	private void canLinkAllOrphanedTest() {
		assertResultIsInvalid(services.linkAllOrphanedPrimitives(admin.getId(), space.getId(),
				TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(
				services.linkAllOrphanedPrimitives(admin.getId(), -1, TestUtil.getMockHttpRequest(admin.getId())));
		assertResultIsInvalid(
				services.linkAllOrphanedPrimitives(-1, space.getId(), TestUtil.getMockHttpRequest(admin.getId())));
	}
	...
	private void recycleOrphanedSolversTest() {
		assertResultIsInvalid(
				services.recycleOrphanedSolvers(admin.getId(), TestUtil.getMockHttpRequest(user.getId())));
	...
	private void editConfigurationTest() {
		...
		assertResultIsInvalid(
				services.editConfigurationDetails(configId, TestUtil.getMockHttpRequest(user.getId(), params)));
		assertResultIsInvalid(services.editConfigurationDetails(configId, TestUtil.getMockHttpRequest(admin.getId())));
		assertResultIsInvalid(
				services.editConfigurationDetails(-1, TestUtil.getMockHttpRequest(admin.getId(), params)));
	}
	...
	private void editSolverTest() {
		...
		params.put("downloadable", "false");
		assertResultIsInvalid(
				services.editSolverDetails(solver.getId(), TestUtil.getMockHttpRequest(user.getId(), params)));
	...
	private void changeJobQueueTest() {
		assertResultIsInvalid(
				services.changeQueueJob(job.getId(), allQ.getId(), TestUtil.getMockHttpRequest(user.getId())));
	...
	private void makePublicTest() {
		assertResultIsInvalid(
				services.makePublic(space.getId(), false, false, TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getUserSolversPaginatedTest() {
		assertResultIsInvalid(
				services.getUserSolversPaginated(admin.getId(), TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getUserRecycledSolversPaginatedTest() {
		assertResultIsInvalid(
				services.getUserRecycledSolversPaginated(admin.getId(), TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getUserBenchmarksPaginatedTest() {
		assertResultIsInvalid(
				services.getUserBenchmarksPaginated(admin.getId(), TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getUserRecycledBenchmarksPaginatedTest() {
		assertResultIsInvalid(
				services.getUserRecycledBenchmarksPaginated(admin.getId(), TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(
				services.getUserRecycledBenchmarksPaginated(-1, TestUtil.getMockHttpRequest(admin.getId())));
	}

	/*
	 * @StarexecTest
	 * private void getTestsPagninatedTest() {
	 * assertResultIsInvalid(services.getTestsPaginated(TestUtil.getMockHttpRequest(
	 * user.getId())));
	 * }
	 */
	...
	private void setLoggingLevelOffForAllExceptClassTest() {
		assertResultIsInvalid(
				services.setLoggingLevelOffForAllExceptClass("trace", "org.starexec.app.RESTServices.java",
						TestUtil.getMockHttpRequest(user.getId())));

		assertResultIsInvalid(
				services.setLoggingLevelOffForAllExceptClass("badlevel", "org.starexec.app.RESTServices.java",
						TestUtil.getMockHttpRequest(admin.getId())));
	 ...
	 * assertResultIsInvalid(services.getTestResultsPaginated(testName,
	 * TestUtil.getMockHttpRequest(user.getId())));
	 * assertResultIsInvalid(services.getTestResultsPaginated(TestUtil.
	 * getRandomAlphaString(50),
	 * TestUtil.getMockHttpRequest(user.getId())));
	 * }
	 */
	...
	private void getPendingCommunityRequestsForCommunityTest() {
		...
		assertResultIsInvalid(
				services.getPendingCommunityRequestsForCommunity(cId, TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(
				services.getPendingCommunityRequestsForCommunity(-1, TestUtil.getMockHttpRequest(admin.getId())));
	}
	...
	private void getGsonPrimitiveSolverTest() {
		assertResultIsInvalid(
				services.getGsonPrimitive(solver.getId(), R.SOLVER, TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getGsonPrimitiveBenchmarkTest() {
		assertResultIsInvalid(
				services.getGsonPrimitive(benchmarkIds.get(0), "bench", TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getGsonPrimitiveSpaceTest() {
		assertResultIsInvalid(
				services.getGsonPrimitive(space.getId(), R.SPACE, TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getGsonPrimitiveProcessorTest() {
		assertResultIsInvalid(services.getGsonPrimitive(postProcessor.getId(), "processor",
				TestUtil.getMockHttpRequest(user.getId())));
	...
	private void getGsonPrimitiveConfigurationTest() {
		...
		assertResultIsInvalid(
				services.getGsonPrimitive(cId, "configuration", TestUtil.getMockHttpRequest(user.getId())));
		assertResultIsInvalid(
				services.getGsonPrimitive(-1, "configuration", TestUtil.getMockHttpRequest(admin.getId())));
	}
	...
	protected void setup() throws Exception {
		...
		admin = loader.loadUserIntoDatabase(TestUtil.getRandomAlphaString(10), TestUtil.getRandomAlphaString(10),
				TestUtil.getRandomPassword(), TestUtil.getRandomPassword(), "The University of Miami",
				R.ADMIN_ROLE_NAME);
