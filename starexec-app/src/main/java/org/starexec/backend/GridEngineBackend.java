...
public class GridEngineBackend implements Backend {
    ...
    // UM edit
    private static final String QUEUE_ASSOC_PATTERN =
        "\\b[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\\b"; // The regular expression to parse out the nodes that belong to a queue from SGE's qstat -f

    public static final String QUEUE_NAME_PATTERN = "QUEUE_NAME";
    public static final String QUEUE_GET_SLOTS_PATTERN =
        "qconf -sq " + QUEUE_NAME_PATTERN; // + " | grep 'slots' | grep -o '[0-9]\\{1,\\}'";

    //    private static final String GRID_ENGINE_PATH = "/cluster/gridengine-8.1.8/bin/lx-amd64/";
    private static final String GRID_ENGINE_PATH =
        R.BACKEND_ROOT + "/bin/lx-amd64/";
    ...
    static {
        ...
        queueAssocPattern = Pattern.compile(
            QUEUE_ASSOC_PATTERN,
    ...
    public int submitScript(
        ...
        try {
            ...
            try {
                ...
                log.error(
                    "[GridEngineBackend.java]: uploading the job produced stderr output: " +
                        e.getMessage() +
                        "."
                );
                return -1;
            ...
            try {
                ...
                if (jobID < 0) {
                    log.error(
                        "[GridEngineBackend.java]: Submitting the job caused an error code: " +
                ...
                log.error(
                    "[GridEngineBackend.java]: there was a problem parsing stdout as an int: stdout was: " +
                        stdout
                );
                return -1;
            }
            ...
            log.error(
                "[GridEngineBackend.java]: Could not find script that was submitted."
            );
            return -1;
        }
    }
    ...
    public boolean killAll() {
        try {
            // UM tomcat
            Util.executeCommand(
                "sudo -u sgeadmin " + GRID_ENGINE_PATH + "qdel -f -u tomcat",
                getSGEEnv()
            );
            return true;
        } catch (Exception e) {
    ...
    public Map<String, String> getNodeQueueAssociations() {
        try {
            ...
            java.util.regex.Matcher matcher =
                GridEngineBackend.queueAssocPattern.matcher(results);
     ...
     * @throws IOException
     */
    public Integer getSlotsInQueue(String queueName)
        throws IOException, StarExecException {
        final String methodName = "getSlotsInQueue";
        try {
            String getSlotsInQueueCommand = QUEUE_GET_SLOTS_PATTERN.replace(
                QUEUE_NAME_PATTERN,
                queueName
            );
            log.debug(
                methodName,
                "Executing command: " + getSlotsInQueueCommand
            );
            ...
            String condensedResults = results
                .trim()
                .replaceAll("\\s+", " ")
                .replaceAll(",", " ");
            log.trace(
                methodName,
                "Condensed Geoff's results: " + condensedResults
            );

            List<String> resultsWords = Arrays.asList(
                condensedResults.split(" ")
            );
            int slotsIndex = resultsWords.indexOf("slots");

            if (slotsIndex == -1) {
                throw new StarExecException(
                    "The result of getting the queue details from SGE did not include a slots attribute."
                );
            }

            if (slotsIndex == resultsWords.size()) {
                throw new StarExecException(
                    "The slots attribute was not followed by a numberal."
                );
            }
            ...
            if (!slots.matches("[0-9]+")) {
                throw new StarExecException(
                    "The slots attribute was not followed by a numeral."
                );
            }
    ...
    public void deleteQueue(String queueName) {
        try {
            ...
            //DISABLE the queue:
            Util.executeCommand(
                "sudo -u sgeadmin " + GRID_ENGINE_PATH + "qmod -d " + queueName,
                getSGEEnv()
            );
            //DELETE the queue:
            Util.executeCommand(
    ...
    public boolean createQueueWithSlots(
        ...
        try {
                ...
                for (int i = 0; i < nodeNames.length; i++) {
                    ...
                    if (sourceQueueName != null) {
                        ...
                        Util.executeCommand(
                            "sudo -u sgeadmin " +
                                GRID_ENGINE_PATH +
                                "qconf -dattr hostgroup hostlist " +
                                fullName +
                                " @" +
                                shortQName +
                                "hosts",
            ...
            Util.executeCommand(
                "sudo -u sgeadmin " +
                    GRID_ENGINE_PATH +
                    "qconf -Ahgrp /tmp/newHost30.hgrp",
                getSGEEnv()
            );
            ...
            if ((nodeNames == null) || (nodeNames.length == 0)) {
                ...
                for (int i = 0; i < nodeNames.length; i++) {
                    ...
                    Util.executeCommand(
                        "sudo -u sgeadmin " +
                            GRID_ENGINE_PATH +
                            "qconf -aattr hostgroup hostlist " +
                            nodeFullName +
                            " @" +
                            shortQueueName +
                            "hosts",
    ...
    public void clearPairTracking(int pairId) {
        // GridEngineBackend does not track pair state - no-op
    }
