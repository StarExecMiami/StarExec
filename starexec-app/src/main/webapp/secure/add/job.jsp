	...
	try {
		...
		if (!GeneralSecurity.hasAdminReadPrivileges(userId) &&
			...
			List<Processor> ListOfPostProcessors =
					Processors.getByCommunity(communityId, ProcessorType.POST);
			List<Processor> ListOfPreProcessors =
					Processors.getByCommunity(communityId, ProcessorType.PRE);
			...
			request.setAttribute(
					"suppressTimestamp", R.SUPPRESS_TIMESTAMP_INPUT_NAME);
...
<star:template title="Run Job in ${space.name}"
	...
	<span id="remainingQuota" style="display:none"
	      value="${remainingPairQuota}"></span>
	...
	<form id="addForm" method="post" action="${starexecRoot}/secure/add/job">
		...
		<fieldset id="fieldStep1">
			...
			<table id="tblConfig" class="shaded contentTbl">
				...
				<tbody>
				<tr class="noHover"
				    title="what default settings profile would you like to use?">
				...
				<tr class="noHover"
				    title="are there any additional details that you want to document with the job?">
					<td class="label"><p>description</p></td>
					<td><textarea length="${jobDescLen}" id="txtDesc"
					              name="desc" rows="6"
					              draggable="false"></textarea></td>
				</tr>
				<tr class="noHover"
				    title="do you want to alter benchmarks before they are fed into the solvers?">
					<td class="label"><p>pre processor</p></td>
					<td>
						<select class="preProcessSetting" id="preProcess"
						        name="preProcess">
				...
				<tr class="noHover"
				    title="do you want to extract any custom attributes from the job results?">
					<td class="label"><p>post processor</p></td>
					<td>
						<select class="postProcessSetting" id="postProcess"
						        name="postProcess">
							<option value="-1">none</option>
				...
				<tr class="noHover"
				    title="which queue should this job be submitted to?">
					...
					<td>
						<select id="workerQueue" name="queue">
							...
							<c:forEach var="q" items="${queues}">
								<option cpumax="${q.cpuTimeout}"
								        wallmax="${q.wallTimeout}"
								        value="${q.id}">${q.name} (${q.id})
								</option>
							</c:forEach>
						</select>
					</td>
				...
				<tr class="noHover"
				    title="the maximum CPU time (in seconds) that each pair can execute before it is terminated (max is any value less than 1)">
				...
				<tr class="noHover"
				    title="the maximum memory usage (in gigabytes) that each pair can use before it is terminated. The minimum of this value and half the available memory on the nodes will be used.">
				...
				<tr class="noHover"
				    title="ask to be notified when the status of this job changes">
					<td class="label"><p>subscribe to job notifications</p></td>
					<td>
						Yes<input type="radio" name="subscribe" value="yes"/>
						No<input type="radio" name="subscribe" value="no"
						         checked="checked"/>
					</td>
				</tr>
				...
				<tr class="noHover advancedOptions"
				    title="How would you like to traverse the job pairs?">
					<td class="label"><p>Job-Pair Traversal</p></td>
					<td>
						Depth-First<input type="radio" id="radioDepth"
						                  name="traversal" value="depth"/>
						Round-Robin<input type="radio" id="radioRobin"
						                  name="traversal" value="robin"/>
					</td>
				</tr>
				<tr class="noHover advancedOptions"
				    title="Would you like to immediately pause the job upon creation?">
					<td class="label"><p>Create Paused</p></td>
					<td>
						Yes<input type="radio" id="radioYesPause" name="pause"
						          value="yes"/>
						No<input type="radio" id="radioNoPause" name="pause"
						         value="no"/>
					</td>
				</tr>
				<tr class="noHover advancedOptions"
				    title="a random value that will be passed into any preprocessor used for this job">
				...
				<tr class="noHover advancedOptions" id="resultsIntervalRow"
				    title="The interval, in seconds, at which to retrieve incremental results for pairs that are running. 0 means results are only obtained after pairs finish. ${MINIMUM_RESULTS_INTERVAL} is the minimum if this is used.">
					<td>
						<p>Results Interval</p>
					</td>
					<td>
						<input type="text" name="resultsInterval"
						       id="resultsInterval" value="0">
					</td>
				</tr>
				<tr class="noHover advancedOptions" id="saveAdditionalOutputRow"
				    title="Whether to save solver output that is placed into the extra output directory given to each solver">
					<td>
						<p>Save Additional Output Files</p>
					</td>
					<td>
						Yes<input type="radio" id="radioYesSaveExtraOutput"
						          name="saveOtherOutput" value="true"/>
						No<input type="radio" id="radioNoSaveExtraOutput"
						         name="saveOtherOutput" value="false"
						         checked="checked"/>
					</td>
				</tr>
				<tr class="noHover advancedOptions runsolveronly"
				    title="whether to include timestamps in the stdout for the pairs in this job">
					<td>
						<p>Suppress Timestamps</p>
					</td>
					<td>
						Yes<input type="radio" id="radioYesSuppressTimestamps"
						          name="${suppressTimestamp}" value="yes"/>
						No<input type="radio" id="radioNoSuppressTimestamps"
						         name="${suppressTimestamp}" value="no"
						         checked="checked"/>
					</td>
				</tr>
		...
		<fieldset id="fieldSolverMethod">
			...
			<table id="tblSpaceSelection" class="contentTbl">
				...
				<tbody>
				<tr id="keepHierarchy">
					<td><input type="hidden" name="runChoice"
					           value="keepHierarchy"/>run and keep hierarchy
						structure
					</td>
					<td>this will run all solvers/configurations on all
						benchmarks in their respective spaces within the space
						hierarchy.
					</td>
				</tr>
				<tr id="runChoose">
					<td><input type="hidden" name="runChoice" value="choose"/>choose
					</td>
					<td>you will choose which solvers/configurations to run
						from ${space.name} only.
					</td>
				</tr>
				</tbody>
			</table>
		...
		<fieldset id="fieldBenchMethod">
			...
			<table id="tblBenchMethodSelection" class="contentTbl">
				...
				<tr id="allBenchInSpace">
					<td><input type="hidden" name="benchChoice"
					           value="runAllBenchInSpace"/>all in ${space.name}
					</td>
					<td>this will run chosen solvers/configurations on all
						benchmarks in ${space.name}</td>
				</tr>
				<tr id="allBenchInHierarchy">
					<td><input type="hidden" name="benchChoice"
					           value="runAllBenchInHierarchy"/>all in hierarchy
					</td>
					<td>this will run chosen solvers/configurations on all
						benchmarks in the hierarchy
					</td>
				</tr>
				<tr id="someBenchInSpace">
					<td><input type="hidden" name="benchChoice"
					           value="runChosenFromSpace"/>choose
						in ${space.name}</td>
					<td>this will run chosen solvers/configurations on your
						selection of benchmarks in the hierarchy
					</td>
				</tr>
				</tbody>
			</table>
