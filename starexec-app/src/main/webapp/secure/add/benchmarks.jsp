...
<%
	try {
		...
		List<Processor> postProcs = Processors
				.getByCommunity(Spaces.getCommunityOfSpace(spaceId),
				                ProcessorType.BENCH
				);
		List<Space> userSpaces = Spaces.getSpacesByUser(userId);
               ...
               <input name="git" type="text"
				...
				<tr>
					...
					<td><input id="radioConvert" type="radio" name="upMethod"
					           value="convert" checked="checked"/> <label
							for="radioConvert"
						...
						file structure to space structure</label> <br/> <input
							id="radioDump"
				...
				<tr
					...
					<td><select id="benchType" name="benchType">
						<c:forEach var="type" items="${types}">
							<option ${type.id == defaultProc ? 'selected' : ''}
									value="${type.id}">${type.name} ${type.id == defaultProc ? '(community default)' : ''}</option>
						</c:forEach>
					</select></td>
				</tr>
				<tr
				...
				<tr id="depLinked">
					...
					<td><input id="linked" type="radio" name="linked"
					           value="true"
					           checked="checked"/><label>yes</label> <input
							id="notLinked" type="radio" name="linked"
							value="false"/><label>no</label></td>
				</tr>
				<tr>
					<td colspan="1">
