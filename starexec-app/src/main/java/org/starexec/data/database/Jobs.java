...
/**
 * Handles all database interaction for jobs (NOT grid engine job execution, see
 * JobManager for that)
 *
 * @author Tyler Jensen
 */
...
public class Jobs {
    ...
    /**
     * Returns a list of job spaces that are present in the given path. Spaces are
     * returned ordered from top level to
    ...
    /**
     * Creates a Space object for use during job creation for any job where
     * benchmarks are going to be saved
     *
     * @param name
     * @param parent
     * @return
     */
    private static Space getNewSpaceForJobCreation(
        String name,
        Space parent,
        int parentId
    ) {
    ...
    /**
     * Given a set job job pairs, creates a set of spaces that mirrors the job space
     * hierarchy
     *
     ...
     */
    public static void createSpacesForPairs(
        List<JobPair> pairs,
        int userId,
        Connection con,
        int parentSpaceId
        ...
        for (JobPair pair : pairs) {
            ...
            for (String name : spaces) {
                ...
                if (!pathsToIds.containsKey(curPathBuilder.toString())) {
                    ...
                    // note that it is assumed that there are no name conflicts here. The security
                    // check is done
                    ...
                    int newId = Spaces.add(
                        con,
                        getNewSpaceForJobCreation(name, parent, parentId),
                        userId
                    );
                    if (newId == -1) {
                        throw new Exception(
                            "error adding new space-- creating spaces for job failed"
                        );
                    }
    ...
    /**
     * Creates all the job spaces needed for a set of pairs. All pairs must have
     * their paths set and they must all be
     * rooted at the same space. Upon return, each pair will have its job space id
     * set to the correct job space
     *
     ...
     * @return The ID of the root job space for this list of pairs, or null on
     *         error.
     * @throws Exception
     */
    public static Integer createJobSpacesForPairs(
        int jobId,
        List<JobPair> pairs,
        Connection con
    ) {
        ...
        for (JobPair pair : pairs) {
            ...
            for (String jobSpaceName : spaces) {
                ...
                if (!pathsToIds.containsKey(curPathBuilder.toString())) {
                    ...
                    parentPath = parentPath.substring(
                        0,
                        parentPath.lastIndexOf(R.JOB_PAIR_PATH_DELIMITER)
                    );
                    pathsToIds.put(
                        curPathBuilder.toString(),
                        Spaces.addJobSpace(jobSpaceName, jobId, con)
                    );
                    ...
                    if (!parentPath.isEmpty()) {
                        ...
                        Spaces.associateJobSpaces(
                            parentId,
                            pathsToIds.get(curPathBuilder.toString()),
                            con
                        );
                    }
                }
                ...
                idsToMaxStages.put(
                    id,
                    Math.max(idsToMaxStages.get(id), pair.getStages().size())
                );
            }
    ...
    /**
     * Makes all new job spaces for the given job and moves all the pairs over to
     * the new spaces. Useful if the job
     * spaces for the given job were somehow corrupted, but path information for the
     * pairs is correct
     *
    ...
    public static int countJobPairsToBeAddedFromConfigIdsForPairedBenchmarks(
        ...
        for (Integer configId : configIds) {
            ...
            for (JobPair pair : jobPairs) {
                // If a pair contains the solver add a new job pair with the new config to the
                // job.
                if (
                    pair.getStages().size() == 1 && // skip multi-stage pairs
                    pair.getPrimaryStage().getSolver().getId() ==
                    solver.getId() &&
    ...
    public static int countJobPairsToBeAddedFromConfigIdsForAllBenchmarks(
        ...
        // Maintain this hashmap that keeps track of which benchmark-solver-config
        // triples we've seen.
        Map<Integer, Map<Integer, Set<Integer>>> jobMap =
            Jobs.getJobMapForPrimaryStage(jobId);
        ...
        for (Integer configIdToAdd : configIds) {
            // Get the solver associated with the config we want to add to the job.
            final int solverIdToAdd = Solvers.getByConfigId(
                configIdToAdd
            ).getId();
            ...
            for (JobPair pair : jobPairs) {
                ...
                if (
                    pair.getStages().size() == 1 &&
                    !jobMapContainsBenchSolverConfigTriple(
                        jobMap,
                        pairBenchId,
                        solverIdToAdd,
                        configIdToAdd
                    ...
                    // Add the new benchmark-solver-config pair so that we don't add it as a
                    // duplicate.
                    addBenchSolverConfigTripleToJobMap(
                        jobMap,
                        pairBenchId,
                        solverIdToAdd,
                        configIdToAdd
                     ...
                     * log.debug(
                     * "Counting job pairs to add, new bench-solver-config triple: "+pairBenchId
    ...
    /**
     * Adds a new job pair using the input list of configurations for each existing
     * job pair in the job that doesn't
     ...
     */
    public static void addJobPairsFromConfigIdsForAllBenchmarks(
        int jobId,
        Set<Integer> configIds
    ) {
        if (isReadOnly(jobId)) {
            log.warn(
                "addJobPairsFromConfigIdsForAllBenchmarks",
                "Job is readonly: " + jobId
            );
            return;
        }
        ...
        for (Integer configIdToAdd : configIds) {
            ...
            // Get new job pairs so that we don't modify a reference we've already added to
            // jobPairsToAdd
            Job job = Jobs.getWithSimplePairs(jobId);
            List<JobPair> jobPairs = job.getJobPairs();
            ...
            for (JobPair pair : jobPairs) {
                ...
                if (
                    ...
                    Configuration configToAdd = Solvers.getConfiguration(
                        configIdToAdd
                    );
        ...
        // Clear the cached job stats for this job so the new job pairs will contribute
        // to the job stats.
        removeCachedJobStats(jobId);
    }

    /**
     * Builds a mapping from all benchmarks to solvers paired with those benchmarks
     * (in job pairs) to configs paired
     * with those benchmarks and solvers (in job pairs) for a given stage. (Only
     * uses pairs for the given stage.)
     *
     * @param jobId       The job to get the pairs from to build the mapping.
     * @param stageNumber The stage number to filter by.
     * @author Albert Giegerich
     */
    public static Map<Integer, Map<Integer, Set<Integer>>> getJobMapForStage(
        int jobId,
        int stageNumber
    ) {
     ...
     * Builds a mapping from all benchmarks to solvers paired with those benchmarks
     * (in job pairs) to configs paired
     * with those benchmarks and solvers (in job pairs) for the primary stage. (Only
     * uses pairs for the primary stage.)
     *
     * @param jobId The job to get the pairs from to build the mapping.
     * @author Albert Giegerich
     */
    ...
    private static Map<Integer, Map<Integer, Set<Integer>>> getJobMap(
        ...
        for (JobPair pair : jobPairs) {
            JoblineStage stage = usePrimaryStage
                ? pair.getPrimaryStage()
                : pair.getStageFromNumber(stageNumber);
            if (stage == null) {
            ...
            if (pairBenchId == 1) {
                log.debug(
                    "Building Map - bench-solver-config: " +
                        pairBenchId +
                        "-" +
                        pairSolverId +
                        "-" +
    ...
    /**
     * Adds a new job pair using the input list of configurations for each existing
     * job pair in the job that contains
     ...
     * @author Albert Giegerich
     */
    public static void addJobPairsFromConfigIdsForPairedBenchmarks(
        int jobId,
        Set<Integer> configIds
    ) {
        List<JobPair> jobPairsToAdd = new ArrayList<>();
        ...
        for (Integer configId : configIds) {
            ...
            // We need a fresh list of job pairs each time so that the references change
            // with each iteration.
            Job job = Jobs.getWithSimplePairs(jobId);
            List<JobPair> jobPairs = job.getJobPairs();
            for (JobPair pair : jobPairs) {
                ...
                if (
                    ...
                    // Modify the current pair by changing the configuration then add the new job
                    // pair to the job.
                    pair
                        .getPrimaryStage()
                        .setConfiguration(Solvers.getConfiguration(configId));
                    jobPairsToAdd.add(pair);
                    benchmarksAlreadySeen.add(pair.getBench().getId());
                }
            }
     ...
     * database, this does not actually submit a
     * job for execution (see JobManager.submitJob). This method also fills in the
     * IDs of job pairs of the given job
     * object.
     *
     * @param job     The job data to add to the database
     * @param spaceId The id of the space to add the job to if pipelines have not
     *                yet been created (most cases) and
        ...
        try {
            ...
            for (StageAttributes attrs : job.getStageAttributes()) {
                if (attrs.getSpaceId() != null) {
                    // make sure that there are no name conflicts when creating the mirrored space
                    // hierarchies.
                    if (
                        Spaces.getSubSpaceIDbyName(
                            attrs.getSpaceId(),
                            rootName,
                            con
                        ) !=
                        ...
                        throw new Exception(
                            "Error creating spaces for job: name conflict with space name " +
                                rootName
                        );
                    }
            ...
            log.debug(
                "job associated, adding this many stage attributes " +
                    job.getStageAttributes().size()
            );
    ...
    /**
     * Adds a job record to the database. This is a helper method for the Jobs.add
     * method
     *
    ...
    private static void addJob(Connection con, Job job) {
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT AddJob(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
            );
    ...
    /**
     * Removes all job database entries where the job has been deleted AND has been
     * orphaned
     *
    ...
    public static boolean cleanOrphanedDeletedJobs() {
        ...
        try {
            ...
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobsAssociatedWithSpaces()"
            );
            results = procedure.executeQuery();
            while (results.next()) {
            ...
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetDeletedJobs()"
            );
            results = procedure.executeQuery();
            ...
            while (results.next()) {
                ...
                if (isReadOnly(jobId)) {
                    log.info(
                        "cleanOrphanedDeletedJobs",
                        "Job " + jobId + " is readonly. Not cleaning."
                    );
                    continue;
                }
                ...
                if (jobDir.exists()) {
                    log.warn(
                        "a deleted job still exists on disk! id = " + jobId
                    );
                    if (!FileUtils.deleteQuietly(jobDir)) {
                        log.warn(
                            "the job could not be deleted! Not removing job from the database"
                        );
                        continue;
                    }
                }
                // the benchmark has been deleted AND it is not associated with any spaces or
                // job pairs
    ...
    public static int countOlderPairs(int jobId, int since) {
        ...
        try {
            con = Common.getConnection();
            stmt = con.prepareStatement("SELECT CountOlderPairs(?,?)");
            stmt.setInt(1, jobId);
            stmt.setInt(2, since);
    ...
    /**
     * Deletes the job with the given id from disk, and permanently removes the job
     * from the database. This is used for
     * testing and is NOT the normal procedure for deleting a job! Call "delete"
     * instead.
     *
     ...
     * Also updates the disk_size and
    ...
    public static boolean setDeletedColumn(int jobId) {
        ...
        try {
            ...
            procedure = con.prepareStatement("SELECT starexec.DeleteJob(?)");
            procedure.setInt(1, jobId);
            procedure.execute();
            return true;
        } catch (Exception e) {
    ...
    /**
     * Deletes the job with the given id from disk, and sets the "deleted" column in
     * the database jobs table to true.
     *
     * @param jobId The ID of the job to delete
     * @return True on success, false otherwise
     */
    public static boolean delete(int jobId) throws SQLException {
        ...
        try {
            ...
            procedure = con.prepareStatement(
                "SELECT starexec.DeleteAllJobPairsInJob(?)"
            );
            procedure.setInt(1, jobId);
            procedure.execute();
    ...
    /**
     * Permanently removes a job from the database. This is a helper function and
     * should NOT be called to delete a job!
    ...
    private static boolean removeJobFromDatabase(int jobId) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT starexec.RemoveJobFromDatabase(?)"
            );
            procedure.setInt(1, jobId);
            procedure.execute();
            return true;
    ...
    public static List<JobPair> getJobPairsToBeDeletedFromConfigIds(
        int jobId,
        Set<Integer> configIds
    ) {
        Job job = Jobs.getWithSimplePairs(jobId);
     ...
     * @author Albert Giegerich
     */
    public static void deleteJobPairsWithConfigurationsFromJob(
        int jobId,
        Set<Integer> configIds
    ) throws SQLException {
        JobPairs.deleteJobPairs(
            getJobPairsToBeDeletedFromConfigIds(jobId, configIds)
        );
        removeCachedJobStatsForConfigs(jobId, configIds);
    }

    /**
     * Gets information about the job with the given ID. Job pair information is not
     * returned. Deleted jobs are not
     * returned.
     *
    ...
    public static boolean addJobStageAttributes(
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT starexec.SetJobStageParams(?,?,?,?,?,?,?,?,?,?,?,?)"
            );
     ...
     */
    public static void setOutputBenchmarksPath(
        final int jobId,
        final String outputBenchmarksDirectory
    ) throws SQLException {
        ...
        try {
            ...
            ps = con.prepareStatement(
                "SELECT starexec.SetOutputBenchmarksPath(?, ?)"
            );
            ps.setInt(1, jobId);
     ...
     * @throws SQLException on database error.
     */
    public static Optional<String> getOutputBenchmarksPath(int jobId)
        throws SQLException {
        return Common.query(
            "SELECT GetOutputBenchmarksPath(?)",
    ...
    public static Job resultsToJob(ResultSet results) throws SQLException {
        ...
        Integer cpuTimeout = ResultSetUtils.getInt(
            results,
            "cpuTimeout",
            "cpu_timeout"
        );
        ...
        Integer clockTimeout = ResultSetUtils.getInt(
            results,
            "clockTimeout",
            "clock_timeout"
        );
        ...
        Long maxMemory = ResultSetUtils.getLong(
            results,
            "maximum_memory",
            "max_memory"
        );
        ...
        Integer softTimeLimit = ResultSetUtils.getInt(
            results,
            "soft_time_limit"
        );
        j.setSoftTimeLimit(softTimeLimit == null ? 0 : softTimeLimit);

        Boolean buildJob = ResultSetUtils.getBoolean(
            results,
            "buildJob",
            "build_job"
        );
        ...
        Boolean suppressTimestamp = ResultSetUtils.getBoolean(
            results,
            "suppressTimestamp",
            "suppress_timestamp"
        );
        ...
        Boolean usingDependencies = ResultSetUtils.getBoolean(
            results,
            "usingDependencies",
            "using_dependencies"
        );
        ...
        String framework = ResultSetUtils.getString(
            results,
        ...
        j.setOutputBenchmarksPath(
            ResultSetUtils.getString(
                results,
                "outputBenchmarksDirectoryPath",
                "output_benchmarks_directory_path"
            )
        );

        Boolean isHighPriority = ResultSetUtils.getBoolean(
            results,
            "isHighPriority",
            "is_high_priority"
        );
    ...
    public static int countPairsByUser(int userId) {
        ...
        try {
            con = Common.getConnection();
            ps = con.prepareStatement(
                "SELECT * FROM starexec.CountPairsByUser(?)"
            );
            ps.setInt(1, userId);
            results = ps.executeQuery();
            if (results.next()) {
                Integer totalPairs = ResultSetUtils.getInt(
                    results,
                    "total_pairs"
                );
    ...
    private static Job get(
        int jobId,
        boolean includeDeleted,
        boolean getSimplePairs
    ) {
        ...
        try {
            ...
            if (includeDeleted) {
                ps = con.prepareStatement(
                    "SELECT * FROM starexec.GetJobByIdIncludeDeleted(?)"
                );
            } else {
                ps = con.prepareStatement(
                    "SELECT * FROM starexec.GetJobById(?)"
                );
            }
     ...
     *
     * This version uses a stored procedure (at its base) that includes configs
     * marked as deleted. Used to construct
     ...
     * @return A list containing every SolverStats for the given job where the
     *         solvers reside in the given space
     * @author Eric Burns
     */

    public static Collection<
        SolverStats
    > getAllJobStatsInJobSpaceHierarchyIncludeDeletedConfigs(
        JobSpace space,
        int stageNumber,
    ...
    public static Map<
        Integer,
        Collection<SolverStats>
    > buildJobSpaceIdToSolverStatsMapWallCpuTimesRounded(
        Job job,
        int stageNumber
    ) {
        Map<Integer, Collection<SolverStats>> outputMap =
            buildJobSpaceIdToSolverStatsMap(job, stageNumber);
        for (Integer jobspaceId : outputMap.keySet()) {
            ...
            for (SolverStats stats : statsList) {
                stats.setWallTime(
                    Math.round(stats.getWallTime() * 100) / 100.0
                );
    ...
    /**
     * Builds a mapping of job space ID's to the stats for the solvers in that job
     * space.
     *
     * @param job         job that owns the job spaces to work on
     * @param stageNumber The stage to filter solver stats by
     * @return a mapping of job space ID's to the stats for the solvers in that job
     *         space
     * @author Albert Giegerich
    ...
    public static Map<
        Integer,
        Collection<SolverStats>
    > buildJobSpaceIdToSolverStatsMap(Job job, int stageNumber) {
    ...
    /**
     * Gets a list of jobs belonging to a space (without its job pairs but with job
     * pair statistics)
     *
    ...
    public static List<Job> getBySpace(int spaceId) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetSpaceJobsById(?)"
            );
    ...
    public static List<Job> getByQueueId(int queueId) throws SQLException {
        ...
        List<Job> r = Common.query(
            "SELECT * FROM starexec.GetQueueJobsById(?)",
    ...
    public static List<Job> getByUserId(int userId) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetUserJobsById(?)"
            );
    ...
    public static int getCountInSpace(int spaceId, String query) {
        ...
        try {
            ...
            stmt = con.prepareStatement(
                "SELECT GetJobCountBySpaceWithQuery(?, ?)"
            );
    ...
    /**
     * Retrieves a job from the database as well as its job pairs that were
     * completed after "since" and its
     ...
     * @return A job object containing information about the requested job, or null
     *         on failure
     * @author Eric Burns
     */
    ...
    private static Job getDetailed(
        ...
        try {
            ...
            if (getCompletedPairsOnly) {
                log.debug(
                    method,
                    "Getting job pairs for job with id=" +
                        jobId +
                        " since completionID=" +
                        since
                );
                j.setJobPairs(
                    Jobs.getNewCompletedPairsDetailed(j.getId(), since)
                );
            } else {
    ...
    /**
     * Returns the absolute path to the directory containing all the log files for
     * the given job
     *
    ...
    private static List<JobPair> getPairsHelper(
        Connection con,
        String sqlMethod,
        int jobId
    ) throws SQLException {
    ...
    protected static List<JobPair> getEnqueuedPairs(Connection con, int jobId)
        ...
        return getPairsHelper(
            con,
            "SELECT * FROM starexec.GetEnqueuedJobPairsByJob(?)",
            jobId
        );
    }
    ...
    /**
     * Retrieves the given job, even if it has been marked as "deleted" in the
     * database. Deep data like job pairs are
     * not populated
     *
    ...
    /**
     * Gets all the the attributes for every job pair in a job, and returns a
     * HashMap mapping pair IDs a map of their
    ...
    > getJobAttributes(Connection con, int jobId) {
        PreparedStatement procedure = null;
        ResultSet results = null;
        log.debug("Getting all attributes for job with ID = " + jobId);
        try {
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobAttrs(?)"
            );
            procedure.setInt(1, jobId);
            results = procedure.executeQuery();
     ...
     *
     * @param jobId The ID of the job in question
     * @return A HashMap mapping integer job-pair IDs to hashmaps that themselves
     *         map jobpair_stage_data ids to
    ...
    public static int getJobCount() {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobCount()"
            );
            results = procedure.executeQuery();
    ...
    public static int getJobCountByUser(int userId) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobCountByUser(?)"
            );
            procedure.setInt(1, userId);
            results = procedure.executeQuery();
    ...
    /**
     * Get the total count of the jobs belong to a specific user that match a
     * specific query
     *
     * @param userId Id of the user we are looking for
     * @param query  The query to match the jobs against
    ...
    public static int getJobCountByUser(int userId, String query) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobCountByUserWithQuery(?, ?)"
            );
            procedure.setInt(1, userId);
     ...
     * @author Eric Burns
     */
    public static int getJobPairCountInJobSpaceByStage(
        int jobSpaceId,
        int stageNumber
    ) {
        Connection con = null;
        ...
        try {
            ...
            procedure = con.prepareStatement(
                "SELECT starexec.GetJobPairCountInJobSpace(?,?) AS jobPairCount"
            );
    ...
    public static int getJobPairCountInJobSpaceByStage(
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT starexec.GetJobPairCountByJobInJobSpaceWithQuery(?, ?,?) AS jobPairCount"
            );
            procedure.setInt(1, jobSpaceId);
            procedure.setString(2, query);
    ...
    /**
     * Retrieves the job pairs necessary to fill the next page of a javascript
     * datatable object, where all the job
     * pairs
     * are in the given job space hierarchy and were operated on by the
     * configuration with the given config ID
     *
     * @param query       A DataTablesQuery object
     * @param jobSpaceId  The ID of the root job space of the job space hierarchy to
     *                    get data for
     ...
     * @param totals      A size 2 int array that, upon return, will contain in the
     *                    first slot the total number of pairs and
     *                    in the second slot the total number of pairs after
     *                    filtering
     * @return A list of job pairs for the given job necessary to fill the next page
     *         of a datatable object
     * @author Eric Burns
     */
    public static List<
        SolverComparison
    > getSolverComparisonsForNextPageByConfigInJobSpaceHierarchy(
        ...
        List<JobPair> pairs = Jobs.getJobPairsInJobSpaceHierarchy(
            jobSpaceId,
            PrimitivesToAnonymize.NONE
        );
        ...
        for (JobPair jp : pairs2) {
            if (benchesToPairs.containsKey(jp.getBench().getId())) {
                try {
                    comparisons.add(
                        new SolverComparison(
                            benchesToPairs.get(jp.getBench().getId()),
                            jp
                        )
                    );
                } catch (Exception e) {
        ...
        comparisons = JobPairs.filterComparisons(
            comparisons,
            query.getSearchQuery()
        );

        totals[1] = comparisons.size();
        SolverComparisonComparator compare = new SolverComparisonComparator(
            query.getSortColumn(),
            wallclock,
            query.isSortASC(),
            stageNumber
        );
        return Util.handlePagination(
            comparisons,
            compare,
            query.getStartingRecord(),
            query.getNumRecords()
        );
    }

    /**
     * Given a list of job pairs, filters and sorts them according to the given
     * parameters and returns the set to
     * display
     *
     * @param pairs       The pairs to filter and sort
     * @param query       Parameters from data table describing which pairs to get
     *                    in which order
     * @param type        The "type" to filter by, where the type refers to the
     *                    different columns of the solver stats table
     * @param wallclock   True to use wallclock time and false to use CPU time
     * @param stageNumber The stage number containing the relevant data, or 0 for
     *                    the primary stage
     * @param totals      A size 2 array that, on exit, will contain the total
     *                    number of pairs after filtering by type and
    ...
    public static List<JobPair> getJobPairsForNextPage(
        ...
        JobPairComparator compare = new JobPairComparator(
            indexOfColumnSortedBy,
            stageNumber,
            query.isSortASC()
        );
        return Util.handlePagination(
    ...
    /**
     * Returns a count of the number of job pairs that satisfy the requirements of
     * the given attributes
     *
     * @param jobSpaceId  The ID of the job space the pairs must be in
     * @param configId    The ID of the configuration the pairs must be using during
     *                    the given stage
     * @param type        The "type" of the pairs as defined by the columns of the
     *                    solver stats table
    ...
    public static int getCountOfJobPairsByConfigInJobSpaceHierarchy(
        ...
        return getCountOfJobPairsByConfigInJobSpaceHierarchy(
            jobSpaceId,
            configId,
            type,
            "",
    ...
    /**
     * Counts the number of job pairs that are in a given job space and use the
     * given configuration and are also of the
     * given "type", which here corresponds to the different columns on the solver
     * stats table in the job details page
     *
     ...
     * @param type        The "type", defined as in the different columns in the
     *                    solver stats table
     * @param query       A query to filter the columns by
     * @param stageNumber The stage number to check
     * @return The total number of pairs that satisfy the given attributes, or -1 on
     *         error
     */
    public static int getCountOfJobPairsByConfigInJobSpaceHierarchy(
        int jobSpaceId,
        ...
        try {
            ...
            stmt.setInt(1, jobSpaceId);
            stmt.setInt(2, configId);
            stmt.setString(3, type);
            stmt.setString(4, query);
            stmt.setInt(5, stageNumber);
            results = stmt.executeQuery();
            if (results.next()) {
                return results.getInt(1);
            }
     ...
     * datatable object, where all the job
     * pairs
     * are in the given space and were operated on by the configuration with the
     * given config ID in the given stage
     *
    ...
    > getJobPairsForNextPageByConfigInJobSpaceHierarchy(
        ...
        return Jobs.getJobPairsForTableInJobSpaceHierarchy(
            jobSpaceId,
            query,
            configId,
            stageNumber,
    ...
    /**
     * If the given string is null, returns a placeholder string. Otherwise, returns
     * the given string
     *
    ...
    /**
     * Gets all the JobPairs in a given job space that were solved by every
     * solver/configuration pair in that space
     *
     * @param jobSpaceId            The ID of the job space to get the pairs for
     * @param stageNumber           The stage number to get data for
     * @param primitivesToAnonymize Object indicating which of solvers and
     *                              benchmarks to anonymize
     * @return All the job pairs in the given job space that are "synchronized" as
     *         defined above
     */
    public static List<JobPair> getSynchronizedPairsInJobSpace(
        ...
        HashSet<String> solverConfigPairs = new HashSet<>(); // will store all the solver/configuration pairs so we know
        // how many there are
        ...
        try {
            ...
            List<JobPair> pairs = Jobs.getJobPairsInJobSpace(
                jobSpaceId,
                stageNumber,
                primitivesToAnonymize
            );
            ...
            for (JobPair p : pairs) {
                solverConfigPairs.add(
                    p.getPrimarySolver().getId() +
                        ":" +
                        p.getPrimaryConfiguration().getId()
                );
                if (benchmarksCount.containsKey(p.getBench().getId())) {
                    benchmarksCount.put(
                        p.getBench().getId(),
                        1 + benchmarksCount.get(p.getBench().getId())
                    );
                } else {
            ...
            // now, we exclude pairs that have benchmarks where the benchmark count is not
            // equal to the solver/config
            // count
    ...
    /**
     * Gets the JobPairs necessary to make the next page of a DataTable of
     * synchronized job pairs in a specific job
     * space
     *
     * @param query                 Parameters from data table describing which
     *                              pairs to get in which order
     * @param jobSpaceId            The ID of the job space containing the pairs
     * @param wallclock             True if we are using wallclock time and false to
     *                              use CPU time
     * @param stageNumber           The stage number to get results for
     * @param primitivesToAnonymize Object indicating which of solvers and
     *                              benchmarks to anonymize
     * @param totals                Must be a size 2 array. The first slot will have
     *                              the number of results before the query, and the
     *                              second slot will have the number of results
     *                              after the query
    ...
    public static List<JobPair> getSynchronizedJobPairsForNextPageInJobSpace(
        ...
        List<JobPair> pairs = Jobs.getSynchronizedPairsInJobSpace(
            jobSpaceId,
            stageNumber,
            primitivesToAnonymize
        );
        return getJobPairsForNextPage(
            pairs,
            query,
            "all",
            wallclock,
    ...
    /**
     * Given the index of a column in the job pairs table on the client side,
     * returns the name of the SQL column we
     * need
     * to sort by
     *
     * @param orderIndex The index of the client side datatable column we are
     *                   sorting on
     * @param wallclock  Whether to use wallclock time or cpu time if we are sorting
     *                   on time.
     * @return The SQL column name
     */
    private static String getJobPairOrderColumn(
        int orderIndex,
        boolean wallclock
    ) {
        switch (orderIndex) {
            ...
            case 6:
                return "job_pairs.id";
            case 7:
                // the - sign is because we want null values last, so we reverse the ASC/ DESC
                // sign and add a -
    ...
    /**
     * Gets the minimal number of Job Pairs necessary in order to service the
     * client's request for the next page of Job
     * Pairs in their DataTables object
     *
     * @param query                 Parameters from data table describing which
     *                              pairs to get in which order
     * @param jobSpaceId            The ID of the job space containing the pairs in
     *                              question
     * @param stageNumber           The stage number to get data for
     * @param wallclock             True to use wallclock time and false to use CPU
     *                              time
     * @param primitivesToAnonymize PrimitivesToAnonymize instance
     * @return a list of 10, 25, 50, or 100 Job Pairs containing the minimal amount
     *         of data necessary
     * @author Todd Elvers
     */
    ...
    public static List<JobPair> getJobPairsForNextPageInJobSpace(
        ...
        try {
            PaginationQueryBuilder builder = new PaginationQueryBuilder(
                PaginationQueries.GET_PAIRS_IN_SPACE_QUERY,
            ...
            results = procedure.executeQuery();
            return getJobPairsForDataTable(
                jobId,
                results,
                false,
                false,
     ...
     */
    public static HashMap<Integer, String> getAllAttrsOfNameForJob(
        int jobId,
        String attrKey
    ) {
        Connection con = null;
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetAttrsOfNameForJob(?,?)"
            );
            procedure.setInt(1, jobId);
    ...
    /**
     * Given a list of job pairs and a ResultSet that contains stages for those
     * pairs, populates the pairs with their
     * stages
     *
     * @param pairs                 The pairs that have stages contained in the
     *                              given result set
     * @param results               The ResultSet containing stages
     * @param getExpectedResult     True to include the expected result column and
     *                              false otherwise
     * @param primitivesToAnonymize an enum describing which (if any) primitive
     *                              names should be anonymized.
     * @return True if the pairs had their stages populated correctly and false
     *         otherwise
     */
    public static boolean populateJobPairStages(
        ...
        try {
            ...
            while (results.next()) {
                ...
                if (jp == null) {
                    log.error(
                        "could not get a pair for id = " +
                            results.getInt("pair_id")
                    );
                    log.error(
                        "id found in mapping = " +
                            idsToPairs.containsKey(results.getInt("pair_id"))
                    );
                    continue;
                }
                ...
                if (id == 0) {
                    ...
                    if (!solvers.containsKey(id)) {
                        ...
                        solve.setId(id);
                        if (
                            AnonymousLinks.areSolversAnonymized(
                                primitivesToAnonymize
                            )
                        ) {
                            solve.setName(
                                results.getString("anon_solver_name")
                            );
                        } else {
                    ...
                    if (!configs.containsKey(id)) {
                        ...
                        if (
                            ...
                            config.setName(
                                results.getString("anon_config_name")
                            );
                        } else {
    ...
    /**
     * Returns all of the job pairs in a given job space, populated with all the
     * fields necessary to display in a
     * SolverStats table. Only the given stage is returned
     *
     * @param jobSpaceId            The space ID of the space containing the solvers
     *                              to get stats for
     * @param stageNumber           The stage number to get data for
     * @param primitivesToAnonymize Object indicating which of solvers and
     *                              benchmarks to anonymize
     * @return A list of job pairs for the given job for which the solver is in the
     *         given space
     * @author Eric Burns
     */
    public static List<JobPair> getJobPairsInJobSpace(
        ...
        PreparedStatement procedure = null;
        log.debug(
            "called getJobPairsInJobSpace with jobSpaceId = " + jobSpaceId
        );
        try {
            ...
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobPairsInJobSpace(?,?,?)"
            );
            ...
            results = procedure.executeQuery();
            log.debug(
                "executing query 1 took " + (System.currentTimeMillis() - a)
            );
            List<JobPair> pairs = processStatResults(
                results,
                true,
                primitivesToAnonymize
            );
            log.debug(
                "processing query 1 took " + (System.currentTimeMillis() - a)
            );
    ...
    /**
     * Returns all of the job pairs in a given job space hierarchy, populated with
     * all the fields necessary to display
    ...
    public static List<JobPair> getJobPairsInJobSpaceHierarchy(
        ...
        return getJobPairsInJobSpaceHierarchy(
            jobSpaceId,
            null,
            primitivesToAnonymize
        );
    ...
    /**
     * Returns all of the successfully completed job pairs in a given job space
     * hierarchy, populated with all the fields necessary to display
     * in a SolverStats table. All job pair stages are obtained
    ...
    public static List<
        JobPair
    > getSuccessfullyCompletedJobPairsInJobSpaceHierarchy(
        int jobSpaceId,
        PrimitivesToAnonymize primitivesToAnonymize
    ) {
        return getSuccessfullyCompletedJobPairsInJobSpaceHierarchy(
            jobSpaceId,
            null,
            primitivesToAnonymize
        );
     ...
     * @param jobSpaceId            The space ID of the space containing the solvers
     *                              to get stats for
     * @param since                 If null, all pairs in the hierarchy are
     *                              returned. Otherwise, only pairs that have a
     *                              completion ID
    ...
    public static List<JobPair> getJobPairsInJobSpaceHierarchy(
        ...
        log.debug(
            methodName,
            "primitivesToAnonymize equals " +
                AnonymousLinks.getPrimitivesToAnonymizeName(
                    primitivesToAnonymize
                )
        );
        try {
            ...
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobPairsInJobSpaceHierarchy(?,?)"
            );
            ...
            Common.safeClose(results);
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobPairStagesInJobSpaceHierarchy(?,?)"
            );
            procedure.setInt(1, jobSpaceId);
            if (since == null) {
                procedure.setNull(2, java.sql.Types.INTEGER);
    ...
    > getSuccessfullyCompletedJobPairsInJobSpaceHierarchy(
        ...
        try {
            ...
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetSuccessfullyCompletedJobPairsInJobSpaceHierarchy(?,?)"
            );
            ...
            Common.safeClose(results);
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetSuccessfullyCompletedJobPairStagesInJobSpaceHierarchy(?,?)"
            );
            procedure.setInt(1, jobSpaceId);
            if (since == null) {
                procedure.setNull(2, java.sql.Types.INTEGER);
    ...
    /**
     * Makes all the job pair objects from a ResultSet formed from querying the
     * database for fields needed in a job
    ...
    private static List<JobPair> getJobPairsForDataTable(
        ...
        try {
            while (results.next()) {
                ...
                if (
                    AnonymousLinks.areBenchmarksAnonymized(
                        primitivesToAnonymize
                    )
                ) {
                ...
                if (
                    AnonymousLinks.areSolversAnonymized(primitivesToAnonymize)
                ) {
                    jp
                        .getPrimarySolver()
                        .setName(results.getString("anon_solver_name"));
                    jp
                        .getPrimaryConfiguration()
                        .setName(results.getString("anon_config_name"));
                } else {
                    jp
                        .getPrimarySolver()
                        .setName(results.getString("solver_name"));
                    jp
                        .getPrimaryConfiguration()
                ...
                jp
                    .getPrimarySolver()
                    .addConfiguration(jp.getPrimaryConfiguration());
    ...
    /**
     * Gets all the job pairs necessary to view in a datatable for a job space. All
     * job pairs returned use the given
     ...
     * @param type        The "type" of the pairs, where type is defined by the
     *                    columns of the solver stats table
    ...
    public static List<JobPair> getJobPairsForTableInJobSpaceHierarchy(
        ...
        try {
            ...
            PaginationQueryBuilder builder = new PaginationQueryBuilder(
                PaginationQueries.GET_PAIRS_IN_SPACE_HIERARCHY_QUERY,
                getJobPairOrderColumn(query.getSortColumn(), false),
                query
            );
            ...
            log.error(
                "getJobPairsForTableInJobSpaceHierarchy",
                "job: " + jobId,
                e
            );
        } finally {
     ...
     * @param primitivesToAnonymize enum designating which (if any) primitive names
     *                              should be anonymized.
     * @param stageNumber           The number of the stage that we are concerned
     *                              with. If <=0, the primary stage is obtained
     * @return A list of size equal to configIds. Each element of the list will
     *         contain a list of job pairs where each
     *         job pair in the list uses the configuration at the matching position
     *         in configIds.
     * @author Eric Burns
     */
    public static List<List<JobPair>> getJobPairsForSolverComparisonGraph(
        ...
        try {
            // we will actually not be using the alternate function calls; we found an
            // easier way to filter out non-correct job pairs
            // List<JobPair> pairs =
            // Jobs.getSuccessfullyCompletedJobPairsInJobSpaceHierarchy(jobSpaceId,
            // primitivesToAnonymize);
            List<JobPair> pairs = Jobs.getJobPairsInJobSpaceHierarchy(
                jobSpaceId,
                primitivesToAnonymize
            ...
            for (JobPair jp : pairs) {
                ...
                if (configToPosition.containsKey(configId)) {
                    List<JobPair> filteredPairs = pairLists.get(
                        configToPosition.get(configId)
                    );
    ...
    public static int getPairCount(int jobId) {
        ...
        try {
            con = Common.getConnection();
            stmt = con.prepareStatement("SELECT countPairsForJob(?)");
            stmt.setInt(1, jobId);
            results = stmt.executeQuery();
    ...
    /**
     * Gets the name of the SQL column to sort on given an index of a dataTables
     * column from the front end
     *
     * @param orderIndex
     * @return
     */
    private static String getJobOrderColumn(int orderIndex) {
        switch (orderIndex) {
            case 0:
                return "jobs.name";
            case 1:
                // this is the same as ordering by status, as the status is determined by
                // whether a job has pending pairs
                return "pendingPairs";
            case 2:
     ...
     * @author Ruoyu Zhang
     */
    public static List<Job> getJobsByUserForNextPage(
        DataTablesQuery query,
        int userId
    ) {
        ...
        try {
            ...
            PaginationQueryBuilder builder = new PaginationQueryBuilder(
                PaginationQueries.GET_JOBS_BY_USER_QUERY,
                getJobOrderColumn(query.getSortColumn()),
                query
            );
    ...
    /**
     * Get next page of the jobs belong to a space, or in the whole system if the ID
     * is -1
     *
     * @param query   A DataTablesQuery object
     * @param spaceId Id of the space we are looking for. If -1, all jobs in the
     *                entire system are returned (for admin
     *                page)
     * @return a list of Jobs belong to the user
     * @author Ruoyu Zhang
     */

    public static List<Job> getJobsForNextPage(
        DataTablesQuery query,
        int spaceId
    ) {
        Connection con = null;
        ...
        try {
            con = Common.getConnection();
            PaginationQueryBuilder builder = new PaginationQueryBuilder(
                PaginationQueries.GET_JOBS_IN_SPACE_QUERY,
                getJobOrderColumn(query.getSortColumn()),
                query
            );
            procedure = new NamedParameterStatement(con, builder.getSQL());
     ...
     * request for the next page of Jobs in
     * their DataTables objects
     *
     * @return a list of 10, 25, 50, or 100 Jobs containing the minimal amount of
     *         data necessary
     * @author Todd Elvers
     */
    private static List<Job> getJobsForNextPage(ResultSet results) {
        ...
        try {
            ...
            while (results.next()) {
                // Grab the relevant job pair statistics; this prevents a secondary set of
                // queries
                // to the database in RESTHelpers.java
                HashMap<String, Integer> liteJobPairStats = new HashMap<>();
                liteJobPairStats.put(
                    "totalPairs",
                    results.getInt("totalPairs")
                );
                liteJobPairStats.put(
                    "completePairs",
                    results.getInt("completePairs")
                );
                liteJobPairStats.put(
                    "pendingPairs",
                    results.getInt("pendingPairs")
                );
                liteJobPairStats.put(
                    "errorPairs",
                    results.getInt("errorPairs")
                );

                Integer completionPercentage = Math.round(
                    (100f * results.getInt("completePairs")) /
                        results.getInt("totalPairs")
                );
                liteJobPairStats.put(
                ...
                Integer errorPercentage = Math.round(
                    (100f * results.getInt("errorPairs")) /
                        results.getInt("totalPairs")
                );
    ...
    public static List<Job> getIncompleteJobs() throws SQLException {
        ...
        try {
            con = Common.getConnection();
            ps = con.prepareStatement(
                "SELECT * FROM starexec.GetIncompleteJobs()"
            );
            rs = ps.executeQuery();
            return Jobs.getJobsForNextPage(rs);
        } finally {
            Common.safeClose(rs);
            Common.safeClose(ps);
            Common.safeClose(con);
    ...
    /**
     * Attempts to retrieve cached SolverStats objects from the database. Returns an
     * empty list if the stats have not
     * already been cached.
     *
    ...
    public static List<SolverStats> getCachedJobStatsInJobSpaceHierarchy(
        ...
        log.debug(
            "calling GetJobStatsInJobSpace with jobspace = " +
                jobSpaceId +
                " and stage = " +
                stageNumber
        );
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobStatsInJobSpace(?,?,?)"
            );
            procedure.setInt(1, jobSpaceId);
            procedure.setInt(2, jobId);
            procedure.setInt(3, stageNumber);
     ...
     * already been cached.
     *
     * This version uses a stored procedure that includes configs marked as deleted.
    ...
    public static List<
        SolverStats
    > getCachedJobStatsInJobSpaceHierarchyIncludeDeletedConfigs(
        int jobSpaceId,
        int stageNumber,
        PrimitivesToAnonymize primitivesToAnonymize,
        boolean includeUnknown
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobStatsInJobSpaceIncludeDeletedConfigs(?,?,?,?)"
            );
            procedure.setInt(1, jobSpaceId);
            procedure.setInt(2, jobId);
            procedure.setInt(3, stageNumber);
            ...
            while (results.next()) {
                ...
                // print status
                log.debug(
                    "in Jobs.getCachedJobStatsInJobSpaceHierarchyIncludeDeletedConfigs:\n" +
                        "config.deleted: " +
                        ResultSetUtils.getBoolean(
                            results,
                            "config.deleted",
    ...
    private static List<JobPair> getAllPairs(int jobId) {
        ...
        try {
            ...
            log.debug(
                methodName,
                "Getting all detailed pairs for job " + jobId
            );

            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetAllJobPairsByJob(?)"
            );
            procedure.setInt(1, jobId);
            results = procedure.executeQuery();
    ...
    /**
     * Gets all job pairs for the given job that have been completed after a given
     * point and also populates its
     * resource
     * TOs. Gets only the primary stage
     *
     * @param jobId The id of the job to get pairs for
     * @param since The completed ID after which to get all jobs
     * @return A list of job pair objects representing all job pairs completed after
     *         "since" for a given job
     * @author Eric Burns
     */
    public static List<JobPair> getNewCompletedPairsDetailed(
        int jobId,
        int since
    ) {
        Connection con = null;
        ...
        try {
            ...
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetNewCompletedJobPairsByJob(?, ?)"
            );
            procedure.setInt(1, jobId);
            procedure.setInt(2, since);
            results = procedure.executeQuery();
            List<JobPair> pairs = getPairsDetailed(jobId, results, true);
            HashMap<Integer, HashMap<Integer, Properties>> props =
                Jobs.getNewJobAttributes(con, jobId, since);

            for (JobPair jp : pairs) {
                if (props.containsKey(jp.getId())) {
                    HashMap<Integer, Properties> pairInfo = props.get(
                        jp.getId()
                    );
                    if (
                        pairInfo.containsKey(
                            jp.getPrimaryStage().getStageNumber()
                        )
                    ) {
                        jp
                            .getPrimaryStage()
                ...
                TreeMap<String, String> jpBenchProps =
                    Benchmarks.getSortedAttributes(jp.getBench().getId());
                ...
                if (jpBenchProps.containsKey(R.EXPECTED_RESULT)) {
                    ...
                    for (JoblineStage stage : jpStages) {
                        ...
                        stage
                            .getAttributes()
                            .setProperty(R.EXPECTED_RESULT, expectedResult);
                    }
                }
            }
    ...
    /**
     * For a given job, gets every job pair with the minimal amount of information
     * required to find the job pair output
     ...
     */
    public static List<JobPair> getNewCompletedPairsShallow(
        int jobId,
        int since
    ) {
        Connection con = null;
        ...
        try {
            ...
            // otherwise, just get the completed ones that were completed later than
            // lastSeen
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetNewJobPairFilePathInfoByJob(?, ?)"
            );
            procedure.setInt(1, jobId);
            procedure.setInt(2, since);
            results = procedure.executeQuery();
            ...
            while (results.next()) {
                ...
                pair.setPrimaryStageNumber(
                    results.getInt("primary_jobpair_data")
                );
                pair.setJobId(jobId);
    ...
    protected static HashMap<
        Integer,
        HashMap<Integer, Properties>
    > getNewJobAttributes(Connection con, int jobId, Integer completionId) {
        PreparedStatement procedure = null;
        ResultSet results = null;
        log.debug("Getting all new attributes for job with ID = " + jobId);
        try {
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetNewJobAttrs(?, ?)"
            );
            procedure.setInt(1, jobId);
    ...
    /**
     * Gets all attributes for every job pair associated with the given job
     * completed after "completionId"
     *
     * @param jobId        The ID of the job in question
     * @param completionId The completion ID after which the pairs are relevant
     * @return A HashMap mapping integer job-pair IDs to Properties objects
    ...
    public static HashMap<
        Integer,
        HashMap<Integer, Properties>
    > getNewJobAttributes(int jobId, int completionId) {
        Connection con = null;
        try {
            con = Common.getConnection();
    ...
    /**
     * Gets all job pairs for the given job non-recursively (simple version) (Worker
     * node, benchmark and solver will
     * NOT
    ...
    protected static List<JobPair> getPairsSimple(Connection con, int jobId) {
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobPairsByJobSimple(?)"
            );
            procedure.setInt(1, jobId);
            results = procedure.executeQuery();
            List<JobPair> returnList = new LinkedList<>();
            ...
            while (results.next()) {
                ...
                c.setName(
                    ResultSetUtils.getString(
                        ...
                        "jobpair_stage_data_config_name",
                        "config_name"
                    )
                );
                s.setId(
                ...
                s.setName(
                    ResultSetUtils.getString(
                        ...
                        "jobpair_stage_data_solver_name",
                        "solver_name"
                    )
                );
    ...
    /**
     * Gets all job pairs for the given job non-recursively (simple version to test
     * job xml bug) (Worker node, status,
    ...
    public static List<JobPair> getPairsPrimaryStageDetailed(int jobId) {
        ...
        try {
            ...
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobPairsPrimaryStageByJob(?)"
            );
            procedure.setInt(1, jobId);
            results = procedure.executeQuery();
            List<JobPair> pairs = getPairsDetailed(jobId, results, false);
            HashMap<Integer, HashMap<Integer, Properties>> props =
                Jobs.getJobAttributes(con, jobId);
            for (JobPair jp : pairs) {
                if (props.containsKey(jp.getId())) {
                    HashMap<Integer, Properties> pairInfo = props.get(
                        jp.getId()
    ...
    /**
     * Gets either all job pairs for the given job and also populates its used
     * resource TOs or only the job pairs that
     * have been completed after the argument "since" Only primary stages are
     * populated (Worker node, status, benchmark
    ...
    private static List<JobPair> getPairsDetailed(
        ...
        try {
            ...
            Hashtable<Integer, Configuration> discoveredConfigs =
                new Hashtable<>();
            ...
            while (results.next()) {
                ...
                // set the completion ID if it exists-- it only exists if we are getting new job
                // pairs
                if (getCompletionId) {
                    jp.setCompletionId(
                        ResultSetUtils.getInt(
                ...
                stage
                    .getSolver()
                    .addConfiguration(discoveredConfigs.get(curConfig));
    ...
    /**
     * Counts the pairs that would be rerun if the user decided to rerun all
     * timeless pairs
     *
    ...
    public static int countTimelessPairs(int jobId) {
        int c1 = Jobs.countTimelessPairsByStatus(
            jobId,
            StatusCode.STATUS_COMPLETE.getVal()
        );
        int c2 = Jobs.countTimelessPairsByStatus(
            jobId,
            StatusCode.EXCEED_CPU.getVal()
        );
        int c3 = Jobs.countTimelessPairsByStatus(
            jobId,
            StatusCode.EXCEED_FILE_WRITE.getVal()
        );
        int c4 = Jobs.countTimelessPairsByStatus(
            jobId,
            StatusCode.EXCEED_MEM.getVal()
        );
        int c5 = Jobs.countTimelessPairsByStatus(
            jobId,
            StatusCode.EXCEED_RUNTIME.getVal()
        );
     ...
     * complete or had a resource out are
     * reset
     *
    ...
    public static boolean setAllPairsToPending(int jobId) {
        ...
        try {
            ...
            if (pairs == null) {
                log.warn(
                    "setAllPairsToPending - getPairsSimple returned null for jobId=" +
                        jobId
                );
                return false;
            }
    ...
    /**
     * Begins the process of rerunning a single pair by removing it from the
     * completed table (if applicable) killing it
    ...
    /**
     * Returns all job pairs in the given job with the given status code that have a
     * run time of 0 for any stage
     *
     * @param jobId      the ID of the job to get pairs for
     * @param statusCode The status code of pairs to search for
     * @return A list of job pair IDs, where each pair has at least one stage with a
     *         run time of 0 and also has the
     *         given status.
     */

    public static List<Integer> getTimelessPairsByStatus(
        int jobId,
        int statusCode
    ) {
        Connection con = null;
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetTimelessJobPairsByStatus(?,?)"
            );
            procedure.setInt(1, jobId);
    ...
    /**
     * Gets all job pair IDs of pairs that have the given status code in the given
     * job, ordered by ID
     *
     * @param jobId      The ID of the job in question
    ...
    public static List<Integer> getPairsByStatus(int jobId, int statusCode) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobPairsByStatus(?,?)"
            );
            procedure.setInt(1, jobId);
            procedure.setInt(2, statusCode);
            results = procedure.executeQuery();
    ...
    /**
     * Gets all job pairs that are pending or were rejected (up to limit) for the
     * given job and also populates its used
     * resource TOs (Worker node, status, benchmark and solver WILL be populated).
     * Gets all stages (except noops)
     *
     * @param con The connection to make the query on
     ...
     * @author TBebnton
     */
    protected static List<JobPair> getPendingPairsDetailed(
        Connection con,
        Job j,
        int limit
    ) {
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetPendingJobPairsByJob(?,?)"
            );
            procedure.setInt(1, j.getId());
            procedure.setInt(2, limit);
            results = procedure.executeQuery();
            // we map ID's to primitives so we don't need to query the database repeatedly
            // for them
            HashMap<Integer, JobPair> pairs = new HashMap<>();
            HashMap<Integer, String> solverIdsToTimestamps = new HashMap<>();
            while (results.next()) {
                try {
                    int currentJobPairId = ResultSetUtils.getInt(
                        results,
                        "job_pairs.id",
                        "job_pairs_id",
                        "id"
                    ...
                    if (pairs.containsKey(currentJobPairId)) {
                        ...
                        Integer dependencyCount = ResultSetUtils.getInt(
                            results,
                            "dependency_count",
                            "benchmarks_dependency_count",
                            "count"
                        );
                        b.setUsesDependencies(
                            dependencyCount != null && dependencyCount > 0
                        );
                        jp.setBench(b);

                        if (j.isUsingDependencies()) {
                            jp.setBenchInputPaths(
                                JobPairs.getJobPairInputPaths(jp.getId(), con)
                            );
                        } else {
                    ...
                    stage.setStageNumber(
                        ResultSetUtils.getInt(results, "stage_number")
                    );
                    stage.setStageId(
                        ResultSetUtils.getInt(results, "stage_id")
                    );
                    jp.addStage(stage);
                    // we need to check to see if the benchId and configId are null, since they
                    // might
                    // have been deleted while the the job is still pending

                    int configId = ResultSetUtils.getInt(
                        results,
                        "jobpair_stage_data.config_id",
                        "jobpair_stage_data_config_id",
                        "config_id"
                    );
                    String configName = ResultSetUtils.getString(
                        results,
                        "jobpair_stage_data.config_name",
                        "jobpair_stage_data_config_name",
                        "config_name"
                    ...
                    stage.setSolver(
                        s /* could be null, if Solver s above was null */
                    );
                    if (s != null) {
                        if (!solverIdsToTimestamps.containsKey(s.getId())) {
                            solverIdsToTimestamps.put(
                                s.getId(),
                                Solvers.getMostRecentTimestamp(con, s.getId())
                            );
                        }
                        s.setMostRecentUpdate(
                            solverIdsToTimestamps.get(s.getId())
                        );
                    }
                } catch (Exception e) {
                    log.error(
                        "getPendingPairsDetailed",
                        "there was an error making a single job pair object",
                        e
                    );
                }
            }
    ...
    /**
     * Returns all the benchmark inputs for all pairs in this job. Format is a
     * HashMap that maps job pair IDs to
     * ordered
     * lists of benchmark IDs, where the order is the input order of the benchmarks
     *
     * @param jobId the ID of the job in question
     * @param con   The open connection to make the call on
     * @return A mapping from jobpair IDs to lists of benchmark IDs, where the
     *         benchmark IDs are ordered according to
     *         their input order for the job pairs
     */
    public static HashMap<Integer, List<Integer>> getAllBenchmarkInputsForJob(
        int jobId,
        Connection con
    ) {
        PreparedStatement procedure = null;
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetAllJobPairBenchmarkInputsByJob(?)"
            );
            procedure.setInt(1, jobId);
            results = procedure.executeQuery();
     ...
     *         where the list is all the benchmark
     ...
     * given job and also populates its used
     * resource TOs (Worker node, status, benchmark and solver WILL be populated).
     * All stages are retrieved
     *
     * @param j     The job to get pairs for. Must have id and using_dependencies
     *              set.
     ...
     * @author Wyatt Kaiser
     */
    protected static List<JobPair> getRunningPairs(Connection con, int jobId)
        throws Exception {
        return getPairsHelper(
            con,
            "SELECT * FROM starexec.GetRunningJobPairsByJob(?)",
            jobId
        );
    }

    /**
     * Gets all job pairs that are running for the given job. Populates only the
     * pair IDs and the SGE Ids
     *
     * @param jobId The id of the job to get pairs for
    ...
    /**
     * Returns the count of pairs with the given status code in the given job where
     * either cpu or wallclock is 0
     *
     * @param jobId
     * @param statusCode
    ...
    private static int countTimelessPairsByStatus(int jobId, int statusCode) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.CountTimelessPairsByStatusByJob(?,?)"
            );
            procedure.setInt(1, jobId);
            procedure.setInt(2, statusCode);
            results = procedure.executeQuery();
    ...
    public static int countPairsByStatus(int jobId, int statusCode) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.CountPairsByStatusByJob(?,?)"
            );
            procedure.setInt(1, jobId);
            procedure.setInt(2, statusCode);
            results = procedure.executeQuery();
    ...
    /**
     * Returns whether the given job has any pairs that are currently waiting to be
     * re post processed.
     *
    ...
    /**
     * Counts the number of pairs a job has that are not complete (status between 1
     * and 6)
     *
     * @param jobId The ID of the job to count pairs for
    ...
    public static int countPendingPairs(int jobId) {
        return Jobs.countPairsByStatus(
            jobId,
            Status.StatusCode.STATUS_PENDING_SUBMIT.getVal()
        );
    }
     ...
     *
     * @param jobId The ID of the job in question
     * @return True if the job is complete, false otherwise (includes the
     ...
     * @param con   The open connection to make the call on
     * @param jobId The ID of the job in question
     * @return True if the job exists in the database with the deleted flag set to
    ...
    public static boolean isJobDeleted(Connection con, int jobId) {
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.IsJobDeleted(?)"
            );
            procedure.setInt(1, jobId);
            results = procedure.executeQuery();
    ...
    /**
     * Determines whether the job with the given ID exists in the database with the
     * column "deleted" set to true
     *
     * @param jobId The ID of the job in question
     * @return True if the job exists in the database and has the deleted flag set
     *         to true
     * @author Eric Burns
     */
    ...
    /**
     * Determines whether the job with the given ID exists in the database with the
     * column "killed" set to true
     *
     * @param jobId The ID of the job in question
     * @return True if the job is killed (i.e. the killed flag is set to true),
     *         false otherwise
     * @author Wyatt Kaiser
     */
    ...
    /**
     * Determines whether the job with the given ID exists in the database with the
     * column "paused" set to true
     *
     * @param jobId The ID of the job in question
     * @return True if the job is paused (i.e. the paused flag is set to true),
     *         false otherwise
     * @author Wyatt Kaiser
     */
    ...
    public static boolean isJobPaused(int jobId) {
        return (
            isJobPausedOrKilled(jobId) == 1 || isJobPausedOrKilled(jobId) == 3
        );
    }
     ...
     * @param con   The open connection to make the query on
     * @param jobId The ID of the job in question
     * @return 0 if the job is neither paused nor killed 1 if the job is paused 2 if
     *         the job has been killed 3 if the
    ...
    public static int isJobPausedOrKilled(Connection con, int jobId) {
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.IsJobPausedOrKilled(?)"
            );
            procedure.setInt(1, jobId);
            results = procedure.executeQuery();
    ...
    /**
     * Determines whether the job with the given ID has either the paused or killed
     * column set to true
     *
     * @param jobId The ID of the job in question
     * @return 0 if the job is neither paused nor killed (or error) 1 if the job is
     *         paused (i.e. the paused flag is set
    ...
    public static List<Integer> getAllJobIds() {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetAllJobIds()"
            );
            results = procedure.executeQuery();
            List<Integer> ids = new ArrayList<>();
            while (results.next()) {
    ...
    /**
     * Returns whether the job is public. A job is public if it was run by the
     * public user or if it is in any public
     * space
     *
     * @param jobId The ID of the job in question
    ...
    public static boolean isPublic(int jobId) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT starexec.JobInPublicSpace(?) AS spaceCount"
            );
            procedure.setInt(1, jobId);
            results = procedure.executeQuery();
            int count = 0;
    ...
    /**
     * kills a running/paused job, and also sets the killed property to true in the
     * database.
     *
    ...
    protected static boolean kill(int jobId, Connection con) {
        PreparedStatement procedure = null;
        try {
            procedure = con.prepareStatement("SELECT starexec.KillJob(?)");
            procedure.setInt(1, jobId);
            procedure.execute();
            ...
            log.debug(
                "deletion of killed job pairs from the queue was successful"
            );
            return true;
        } catch (Exception e) {
            log.error("kill", e);
     ...
     * @author Albert Giegerich
     */
    protected static void killUsersJobs(int userId)
        ...
        try {
            ...
            throw new StarExecDatabaseException(
                "Database error while deleting all jobs owned by user with id=" +
                    userId,
                e
            );
        } finally {
    ...
    protected static boolean pause(int jobId, Connection con) {
        ...
        try {
            ...
            procedure = con.prepareStatement("SELECT starexec.PauseJob(?)");
            procedure.setInt(1, jobId);
            procedure.execute();
    ...
    /**
     * pauses all running jobs (via admin page), and also sets the paused &
     * paused_admin to true in the database.
     *
     * @return True on success, false otherwise
     * @author Wyatt Kaiser
     */
        ...
        try {
            ...
            if (jobs != null) {
                for (Integer jobId : jobs) {
                    // Get the enqueued job pairs and remove them
                    try {
                        List<JobPair> jobPairsEnqueued = Jobs.getEnqueuedPairs(
                            jobId
                        );
                        ...
                        log.warn(
                            "Caught SQLException while getting enqueued pairs."
                        );
                    }
                    ...
                    log.debug(
                        "Deletion of paused job pairs from queue was successful"
                    );
                }
            }
     ...
     * @throws StarExecDatabaseException
     */
    public static void setJobName(int jobId, String newName)
        throws StarExecDatabaseException {
        ...
        try {
            ...
            procedure = connection.prepareStatement(
                "SELECT starexec.SetJobName(?, ?)"
            );
            procedure.setInt(1, jobId);
            ...
            throw new StarExecDatabaseException(
                "Could not save job name to database.",
                e
            );
        } finally {
     ...
     * @throws StarExecDatabaseException
     */
    public static void setJobDescription(int jobId, String newDescription)
        throws StarExecDatabaseException {
        ...
        try {
            connection = Common.getConnection();
            procedure = connection.prepareStatement(
                "SELECT starexec.SetJobDescription(?, ?)"
            );
            procedure.setInt(1, jobId);
            ...
            log.error("Caught exception.", e);
            throw new StarExecDatabaseException(
                "Could not save job description to database.",
                e
            );
        } finally {
            Common.safeClose(connection);
    ...
    /**
     * Given a set of pairs and a mapping from pair IDs, to stage numbers to
     * properties, loads the properties into the
     * appropriate pairs
     *
     * @param pairs The job pairs to load attributes into
     * @param attrs A HashMap that maps job pair IDs to a second map that goes from
     *              stage numbers to Properties.
     */
    public static void loadPropertiesIntoPairs(
        List<JobPair> pairs,
        HashMap<Integer, HashMap<Integer, Properties>> attrs
    ) {
        for (JobPair jp : pairs) {
            ...
            if (stageAttrs != null) {
                for (JoblineStage stage : jp.getStages()) {
                    if (stageAttrs.containsKey(stage.getStageNumber())) {
                        stage.setAttributes(
                            stageAttrs.get(stage.getStageNumber())
                        );
                    }
                }
            }
     ...
     * returns a hashmap mapping job pair
     * ids to
    ...
    private static HashMap<
        Integer,
        HashMap<Integer, Properties>
    > processAttrResults(ResultSet results) {
        try {
            HashMap<Integer, HashMap<Integer, Properties>> props =
                new HashMap<>();
            int id;
    ...
    private static void addStageToSolverStats(
        SolverStats stats,
        JoblineStage stage,
        boolean includeUnknown
    ) {
     ...
     * @author Eric Burns
     */
    public static Collection<SolverStats> processPairsToSolverStats(
        int jobId,
        List<JobPair> pairs,
        boolean includeUnknown
    ) {
        ...
        try {
            ...
            for (JobPair jp : pairs) {
                for (JoblineStage stage : jp.getStages()) {
                    ...
                    // // print status
                    // log.debug( "in Jobs.processPairsToSolverStats(): current configId = " +
                    // stage.getConfiguration().getId() +
                    // "; current deleted status = " + stage.getConfiguration().getDeleted() );

                    int stageNumber = stage.getStageNumber();
                    Integer conflicts = null;
                    if (!stats.containsKey(key)) {
                        // current stats entry does not yet exist
                        ...
                        // Compute the number of conflicts and save them in variable in case we need to
                        // use them again.
                        conflicts = Solvers.getConflictsForConfigInJobWithStage(
                            jobId,
                            configId,
                            stageNumber
                        );
                    ...
                    if (
                        stage
                            .getStageNumber()
                            .equals(jp.getPrimaryStageNumber())
                    ) {
                        // if we get here, we need to add this stage to the primary stats as well
                        key =
                            0 +
                            ":" +
                            String.valueOf(stage.getConfiguration().getId());
                        if (!stats.containsKey(key)) {
                            // current stats entry does not yet exist
                            SolverStats newSolver = new SolverStats();
                            newSolver.setStageNumber(0);
                            newSolver.setSolver(stage.getSolver());
                            newSolver.setConfiguration(
                                stage.getConfiguration()
                            );
                            if (conflicts == null) {
                                conflicts =
                                    Solvers.getConflictsForConfigInJobWithStage(
                                        jobId,
                                        configId,
                                        stageNumber
                                    );
            ...
            log.debug(
                methodName,
                "Time taken to process job pairs to stats for job with " +
                    Jobs.getPairCount(jobId) +
                    " pairs: " +
                    stopWatch.toString()
            );
            return stats.values();
    ...
    /*
     * Gets all of the benchmarks in a job for which a job pair gave a result that
     * conflicted with another job pair on
     * the same benchmark.
     *
     ...
     *
     * public static Set<Integer> getConflictingBenchmarksForJob(int jobId) throws
     * SQLException {
     ...
     * log.
     * debug("Benchmarks found in job while searching for conflicting benchmarks: "
     * + benchmarksInJob.size());
     *
     * benchmarkLoop:
     * for (Benchmark benchmarkInJob : benchmarksInJob) {
     * // Loop through all the job pairs containing the benchmark. If two gave
     * different results then the
     * benchmark
     * // is conflicting.
     * String firstResultFound = null;
     * List<JobPair> jobPairsInJobContainingBenchmark =
     * JobPairs.getPairsInJobContainingBenchmark(jobId,
     ...
     * // Since there were two different results, add the benchmark to conflicting
     * benchmarks and
     ...
     * }
     * stopWatch.stop();
     * log.debug("Time taken to get conflicting benchmarks for job with "+Jobs.
     * getPairCount(jobId)+" pairs:
     * "+stopWatch.toString());
    ...
    /**
     * Given the result set from a SQL query containing job pair info, produces a
     * list of job pairs for which all the
        ...
        try {
            ...
            while (results.next()) {
                ...
                if (includeSingleStage) {
                    ...
                    // a table.
                    // so, we simply set the primary stage of this pair to the first stage for the
                    // time being
                    jp.setPrimaryStageNumber(1);
                    JoblineStage stage = new JoblineStage();
                    stage.setStageNumber(1);
                    Double cpu = ResultSetUtils.getDouble(
                        results,
                        "jobpair_stage_data.cpu",
                        "jobpair_stage_data_cpu",
                        "cpu"
                    );
                    if (cpu != null) {
                        stage.setCpuUsage(cpu);
                    }
                    Double wallclock = ResultSetUtils.getDouble(
                        results,
                        "jobpair_stage_data.wallclock",
                        "jobpair_stage_data_wallclock",
                        "wallclock"
                    );
                    if (wallclock != null) {
                    ...
                    stage.setStageId(
                        ResultSetUtils.getInt(
                            ...
                            "jobpair_stage_data_stage_id",
                            "stage_id"
                        )
                    );
                    Integer statusCode = ResultSetUtils.getInt(
                        results,
                        "jobpair_stage_data.status_code",
                        "jobpair_stage_data_status_code",
                        "status_code"
                    );
                    if (statusCode != null) {
                    ...
                    // everything below this line is in a stage
                    Integer solverId = ResultSetUtils.getInt(
                        results,
                        "jobpair_stage_data.solver_id",
                        "jobpair_stage_data_solver_id",
                        "solver_id"
                    ...
                    if (solverId == null || solverId == 0) {
                        ...
                        Integer configId = ResultSetUtils.getInt(
                            results,
                            "jobpair_stage_data.config_id",
                            "jobpair_stage_data_config_id",
                            "config_id"
                        );
                        Integer effectiveConfigId = configId == null
                            ? 0
                            : configId;
                        ...
                        stage
                            .getSolver()
                            .addConfiguration(configs.get(effectiveConfigId));
    ...
    /**
     * Resumes a paused job, and also sets the paused property to false in the
     * database.
     *
    ...
    protected static boolean resume(int jobId, Connection con) {
        ...
        PreparedStatement procedure = null;
        try {
            procedure = con.prepareStatement("SELECT starexec.ResumeJob(?)");
            procedure.setInt(1, jobId);
            procedure.execute();
    ...
    /**
     * resumeAll sets global pause to false, which allows job pairs to be sent to
     * the grid engine again
     *
     * @return true on success and false otherwise
     * @author Wyatt Kaiser
     */
    public static boolean resumeAll() {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement("SELECT starexec.ResumeAll()");
            procedure.execute();
    ...
    /**
     * Sets the given job up to be post processed by adding all of its pairs to the
     * processing_job_pairs table
     *
     ...
     * @author Eric Burns
     */
    public static boolean prepareJobForPostProcessing(
        int jobId,
        int processorId,
        int stageNumber
    ) {
        ...
        try {
            ...
            if (!Jobs.removeCachedJobStats(jobId, con)) {
                throw new Exception(
                    "Couldn't clear out the cache of job stats"
                );
            }

            procedure = con.prepareStatement(
                "SELECT starexec.PrepareJobForPostProcessing(?,?,?,?,?)"
            );
            procedure.setInt(1, jobId);
    ...
    /**
     * If the job is not yet complete, does nothing, as we don't want to store stats
     * for incomplete jobs.
     *
    ...
    public static void saveStats(
        ...
        if (!isJobComplete(jobId)) {
            log.debug(
                "stats for job with id = " +
                    jobId +
                    " were not saved because the job is incomplete"
            );
        ...
        try {
            ...
            for (SolverStats s : stats) {
                if (!saveStats(s, con, includeUnknown)) {
                    throw new Exception(
                        "saving stats failed, rolling back connection"
                    );
                }
            }
        } catch (Exception e) {
    ...
    /**
     * Given a SolverStats object, saves it in the database so that it does not need
     * to be generated again This
     * function
     * is currently called only when the job is complete, as we do not want to cache
     * stats for incomplete jobs.
     *
    ...
    private static boolean saveStats(
        ...
        try {
            ...
            log.error(
                "caught an exception while trying to save a single stat: " +
                    e.getMessage()
            );
        } finally {
            Common.safeClose(procedure);
    ...
    /**
     * Updates the primary space of a job. This should only be necessary when
     * changing the primary space of an older
     * job
     ...
     * @author Eric Burns
     */
    private static void updatePrimarySpace(
        int jobId,
        int jobSpaceId,
        Connection con
    ) {
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT starexec.UpdatePrimarySpace(?, ?)"
            );
            procedure.setInt(1, jobId);
     ...
     * @author Albert Giegerich
     */
    public static void removeCachedJobStatsForConfigs(
        int jobId,
        Set<Integer> configIds
    ) throws SQLException {
        Connection con = null;
        try {
            ...
            List<JobSpace> jobSpacesInJob = Spaces.getSubSpacesForJob(
                rootSpaceId,
                true
            );
            ...
            for (JobSpace jobSpace : jobSpacesInJob) {
                for (int cid : configIds) {
                    removeCachedJobStatsForConfigAndJobSpace(
                        con,
                        jobSpace.getId(),
                        cid
                    );
    ...
    private static void removeCachedJobStatsForConfigAndJobSpace(
        Connection con,
        int jobSpaceId,
        int configId
    ) throws SQLException {
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT starexec.RemoveJobStatsInJobSpaceForConfig(?, ?)"
            );
            procedure.setInt(1, jobSpaceId);
    ...
    public static boolean removeCachedJobStats(int jobId, Connection con) {
        ...
        try {
            ...
            for (JobSpace s : jobSpaces) {
                procedure = con.prepareStatement(
                    "SELECT starexec.RemoveJobStatsInJobSpace(?)"
                );
    ...
    public static boolean removeAllCachedJobStats(Connection con) {
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT starexec.RemoveAllJobStats()"
            );
            procedure.execute();
            Common.safeClose(procedure);
    ...
    public static int getPausedJobCount() {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetPausedJobCount()"
            );
            results = procedure.executeQuery();
    ...
    public static int getRunningJobCount() {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetRunningJobCount()"
            );
            results = procedure.executeQuery();
    ...
    /**
     * Returns all jobs owned by the given user that have pairs either running or
     * pending
     *
    ...
    public static List<Integer> getRunningJobs(int userId) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetRunningJobsByUser(?)"
            );
            procedure.setInt(1, userId);
            results = procedure.executeQuery();
     ...
     * and which are not currently paused
     * or killed
     *
    ...
    public static List<Integer> getRunningJobs() {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetRunningJobs()"
            );
            results = procedure.executeQuery();
    ...
    public static boolean isSystemPaused() {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.IsSystemPaused()"
            );
            results = procedure.executeQuery();
    ...
    public static List<Integer> getOrphanedJobs(int userId) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetOrphanedJobIds(?)"
            );
            procedure.setInt(1, userId);
            results = procedure.executeQuery();
            while (results.next()) {
    ...
    /**
     * Given a ResultSet that is currently pointing to a row containing data for a
     * StageAttributes object, generates
     * the
     * object
     *
     * @param results The results, which must be pointing to a row with a
     *                StageAttributes object
    ...
    public static StageAttributes resultsToStageAttributes(ResultSet results) {
        try {
            ...
            attrs.setStdoutSaveOption(
                SaveResultsOption.valueOf(results.getInt("stdout_save_option"))
            );
            attrs.setExtraOutputSaveOption(
                SaveResultsOption.valueOf(
                    results.getInt("extra_output_save_option")
                )
            );
            return attrs;
        } catch (Exception e) {
     ...
     */
    public static List<StageAttributes> getStageAttrsForJob(
        int jobId,
        Connection con
    ) {
        ResultSet results = null;
        ...
        try {
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.getStageParamsByJob(?)"
            );
            procedure.setInt(1, jobId);
            results = procedure.executeQuery();
            ...
            while (results.next()) {
                ...
                a.setPostProcessor(
                    Processors.get(results.getInt("post_processor"))
                );
                a.setPreProcessor(
                    Processors.get(results.getInt("pre_processor"))
                );
                attrs.add(a);
            }
    ...
    /**
     * This function takes all job pairs that 1) Have status code 2-5, meaning they
     * should be enqueued or running 2)
     * Are
     * not currently listed in the backend and sets them to status code 9. This
     * basically takes pairs that have somehow
     ...
     * @throws IOException
     */
    public static void setBrokenPairsToErrorStatus(Backend backend)
        throws IOException {
    ...
    public static boolean doesJobCopyBackIncrementally(int jobId) {
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.DoesJobCopyBackIncrementally(?)"
            );
            procedure.setInt(1, jobId);
            results = procedure.executeQuery();
            if (results.next()) {
                jobCopiesBackResultsIncrementally = results.getBoolean(
                    "jobCopiesBackIncrementally"
                );
            }
        } catch (Exception e) {
        ...
        return jobCopiesBackResultsIncrementally == null
            ? false
            : jobCopiesBackResultsIncrementally;
    }

    public static List<String> getJobAttributeValues(int jobSpaceId)
        throws SQLException {
        java.sql.Connection con = null;
        java.sql.PreparedStatement ps = null;
        ...
        try {
            con = Common.getConnection();
            ps = con.prepareStatement(
                "SELECT * FROM starexec.GetJobAttributesTableHeaders(?)"
            );
    ...
    /**
     * Sets a job to be low priority meaning the user's other jobs should be run
     * first.
     *
    ...
    /**
     * Sets a job to be high priority meaning this job should run before other's
    ...
    /**
     * Gets the slots in a job's queue if the backend is SGE, otherwise just returns
    ...
    public static String getSlotsInJobQueue(Job job) {
        ...
        if (R.BACKEND_TYPE.equals(R.SGE_TYPE)) {
            ...
            try {
                Integer slots = backend.getSlotsInQueue(
                    job.getQueue().getName()
                );
                ...
                log.error(
                    methodName,
                    "SGE may not have returned an integer when queried.",
                    e
                );
            }
        }
    ...
    public static List<AttributesTableData> getJobAttributesTable(
        int jobSpaceId
    ) {
        Connection con = null;
        PreparedStatement procedure = null;
        ...
        try {
            con = Common.getConnection();
            procedure = con.prepareStatement(
                "SELECT * FROM starexec.GetJobAttributesTable(?)"
            );
            procedure.setInt(1, jobSpaceId);
            results = procedure.executeQuery();
            while (results.next()) {
     ...
     * @throws SQLException
     */
    public static List<Triple<String, Integer, TimePair>> getJobAttributeTotals(
        int jobspaceId
    ) throws SQLException {
        java.sql.Connection con = null;
        java.sql.PreparedStatement ps = null;
        ...
        try {
            con = Common.getConnection();
            ps = con.prepareStatement(
                "SELECT * FROM starexec.GetSumOfJobAttributes(?)"
            );
            ps.setInt(1, jobspaceId);
            results = ps.executeQuery();
            List<Triple<String, Integer, TimePair>> valueCounts =
                new ArrayList<>();
            while (results.next()) {
                valueCounts.add(
                    new ImmutableTriple<>(
