<%@page contentType="text/html" pageEncoding="UTF-8"
        import="org.starexec.data.database.Jobs, org.starexec.data.database.Solvers, org.starexec.data.security.JobSecurity, org.starexec.data.to.Benchmark, org.starexec.data.to.Configuration, org.starexec.data.to.Solver, org.starexec.util.SessionUtil, java.util.List" %>
...
<%
	try {
		...
		List<Benchmark> conflictingBenchmarksForSolverConfig =
				Solvers.getConflictingBenchmarksInJobForStage(jobId, configId,
				                                              stageNumber
				);
		request.setAttribute(
				"conflictingBenchmarks", conflictingBenchmarksForSolverConfig);

		// Get the solver even if it's deleted since this is for job stats.
		boolean getSolverEvenIfDeleted = true;
		Solver solver =
				Solvers.getSolverByConfig(configId, getSolverEvenIfDeleted);
