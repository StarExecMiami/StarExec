		...
		try {
			...
			if (pairsAdded > 0 && (remainingQuota + netPairs) < 0) {
				response.sendError(
						...
						"You do not have sufficient job pair quota to add these pairs"
				);
				return;
			}
			if (pairsAdded > 0 && Users.isDiskQuotaExceeded(userId)) {
				response.sendError(HttpServletResponse.SC_BAD_REQUEST,
				                   "Your disk quota has been exceeded: please clear out some old solvers, jobs, or " +
						                   "benchmarks before proceeding");
				return;
			}
			...
			if (!(Jobs.isJobPaused(jobId) || Jobs.isJobComplete(jobId))) {
				response.sendError(
						HttpServletResponse.SC_FORBIDDEN, "Job must be finished or paused to add job pairs.");
				return;
			}
			...
			// Filter the selected config ids into two sets, one that contains config ids that we want to pair with
			// every benchmark, and one
			// that contains config ids that we want to pair with every benchmark already paired with the config's
			// solver in the job.
