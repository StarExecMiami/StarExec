...
public class OARBackend implements Backend {
    ...
    public boolean killAll() {
        try {
            for (Integer i : this.getActiveExecutionIds()) {
                if (!killPair(i)) {
                    log.error(
                        "ERROR: Unable to kill pair with execution id: " + i
                    );
                }
            }
            return true;
    ...
    public boolean createQueue(
        ...
        try {
            Util.executeCommand(
                "oarnotify --add_queue " +
                    newQueueName +
                    ",1,oar_sched_gantt_with_timesharing"
            );
    ...
    public void clearPairTracking(int pairId) {
        // OARBackend does not track pair state - no-op
    }
    ...
    public Set<Integer> getActiveExecutionIds() throws IOException {
        try {
            ...
            for (Entry<String, JsonElement> s : object.entrySet()) {
                ids.add(
                    s.getValue().getAsJsonObject().get("Job_Id").getAsInt()
                );
            }
            return ids;
