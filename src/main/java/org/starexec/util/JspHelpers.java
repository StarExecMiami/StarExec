...
public class JspHelpers {
	...
	public static void handleJobPage(HttpServletRequest request, HttpServletResponse response)
		...
		if (isAnonymousPage) {
			...
			Optional<PrimitivesToAnonymize> potentialPrimitivesToAnonymize = AnonymousLinks
					.getPrimitivesToAnonymizeForJob(jobUuid);
			if (potentialJobId.isPresent() && potentialPrimitivesToAnonymize.isPresent()) {
				...
				// Give the anonymous user the ability to view the job as if they were the
				// owning user.
				// This won't affect the user id in the session and therefore won't cause any
				// security issues.
		...
		if (Permissions.canUserSeeJob(jobId, userId).isSuccess()) {
			...
			if (jobSpaceId > 0) {
				...
				// Determine which pipeline stage to display; default to 1 if not provided or
				// invalid.
				...
				if (isLocalJobPage) {
					Map<Integer, String> jobSpaceIdToSubspaceJsonMap = RESTHelpers
							.getJobSpaceIdToSubspaceJsonMap(j.getId(), jobSpaces);
					request.setAttribute("jobSpaceIdToSubspaceJsonMap", jobSpaceIdToSubspaceJsonMap);

					Map<Integer, String> jobSpaceIdToCpuTimeSolverStatsJsonMapExcludeUnknowns = RESTHelpers
							.getJobSpaceIdToSolverStatsJsonMap(jobSpaces, selectedStageNumber, false, false);
					request.setAttribute("jobSpaceIdToCpuTimeSolverStatsJsonMapExcludeUnknowns",
							jobSpaceIdToCpuTimeSolverStatsJsonMapExcludeUnknowns);

					Map<Integer, String> jobSpaceIdToWallclockTimeSolverStatsJsonMapExcludeUnknowns = RESTHelpers
							.getJobSpaceIdToSolverStatsJsonMap(jobSpaces, selectedStageNumber, true, false);
					// used to creat the panels for subspace summaries
					request.setAttribute("jobSpaceIdToWallclockTimeSolverStatsJsonMapExcludeUnknowns",
							jobSpaceIdToWallclockTimeSolverStatsJsonMapExcludeUnknowns);
					Map<Integer, String> jobSpaceIdToCpuTimeSolverStatsJsonMapIncludeUnknowns = RESTHelpers
							.getJobSpaceIdToSolverStatsJsonMap(jobSpaces, selectedStageNumber, false, true);
					request.setAttribute("jobSpaceIdToCpuTimeSolverStatsJsonMapIncludeUnknowns",
							jobSpaceIdToCpuTimeSolverStatsJsonMapIncludeUnknowns);

					Map<Integer, String> jobSpaceIdToWallclockTimeSolverStatsJsonMapIncludeUnknowns = RESTHelpers
							.getJobSpaceIdToSolverStatsJsonMap(jobSpaces, selectedStageNumber, true, true);
					// used to creat the panels for subspace summaries
					request.setAttribute("jobSpaceIdToWallclockTimeSolverStatsJsonMapIncludeUnknowns",
							jobSpaceIdToWallclockTimeSolverStatsJsonMapIncludeUnknowns);
					Map<Integer, List<JobPair>> jobSpaceIdToPairMap = JobPairs
							.buildJobSpaceIdToJobPairMapWithWallCpuTimesRounded(j);
				...
				Processor selectedStagePostProc = j.getStageAttributesByStageNumber(selectedStageNumber)
						.getPostProcessor();
				Processor selectedStagePreProc = j.getStageAttributesByStageNumber(selectedStageNumber)
						.getPreProcessor();
				...
				if (forwardedHost != null) {
					...
					starexecUrl = String.format("%s://%s%s/%s/", R.STAREXEC_URL_PREFIX, R.STAREXEC_SERVERNAME, port,
							R.STAREXEC_APPNAME);
				}
				request.setAttribute("starexecUrl", starexecUrl);
			} else {
				response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
						"The details for this job could not be obtained");
			}
	 ...
	 */
	public static void handleAnonymousSolverPage(String uniqueId, HttpServletRequest request,
		...
		try {
			...
			Optional<PrimitivesToAnonymize> primitivesToAnonymize = AnonymousLinks
					.getPrimitivesToAnonymizeForSolver(uniqueId);
			...
			if (primitivesToAnonymize.isPresent() && solverId.isPresent()) {
				...
				setSolverPageRequestAttributes(true, AnonymousLinks.areSolversAnonymized(primitivesToAnonymize.get()),
						solver, request);
			} else {
	...
	/**
	 * Handles request/response logic for details/solver.jsp for non-anonymous
	 * pages.
	 * 
	 * @author Albert Giegerich
	 */
	public static void handleNonAnonymousSolverPage(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
	...
	private static void setSolverPageRequestAttributes(
		...
		// we need two versions of every website URL-- one for insertion into an
		// attribute and
		// one for insertion into the HTML body. This data structure represents every
		// site with 3 strings
	...
	private static void handleNullSolver(int solverId, HttpServletResponse response) throws IOException {
		if (Solvers.isSolverDeleted(solverId)) {
			...
			response.sendError(HttpServletResponse.SC_NOT_FOUND,
					"This solver has been moved to the trash bin by its owner.");
		} else {
			response.sendError(HttpServletResponse.SC_NOT_FOUND, "Solver does not exist or is restricted");
		}
	}
	 ...
	 * @author Albert Giegerich
	 */
	public static void handleAnonymousBenchPage(String uniqueId, HttpServletRequest request,
			HttpServletResponse response)
			throws IOException, SQLException {
		Optional<Integer> benchmarkId = AnonymousLinks.getIdOfBenchmarkAssociatedWithLink(uniqueId);
		Optional<PrimitivesToAnonymize> primitivesToAnonymize = AnonymousLinks
				.getPrimitivesToAnonymizeForBenchmark(uniqueId);
	...
	public static void handleNonAnonymousBenchPage(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
	...
	public static void handleAnonymousJobKeyPage(HttpServletRequest request, HttpServletResponse response)
			throws IOException, SQLException {
		int userId = SessionUtil.getUserId(request);
		...
		if (!JobSecurity.userOwnsJobOrIsAdmin(jobId, userId)) {
			response.sendError(HttpServletResponse.SC_FORBIDDEN,
					"You are not permitted to see this job's anonymous solver name key page.");
			return;
		}

		Job job = Jobs.get(jobId);
		List<Triple<String, String, Integer>> anonymousSolverNamesKey = AnonymousLinks
				.getAnonymousSolverNamesKey(jobId);
	...
	private static void setBenchmarkPageRequestAttributes(
		...
		if (!isAnonymousPage) {
			...
			try {
				...
				response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
						"Error occurred while checking for broken benchmark dependencies.");
				return;
			}
		}
		request.setAttribute("downloadable", downloadable);
		...
		// Get the benchmark, if it can't be gotten send an error message telling the
		// user why.
		...
		StringBuilder js = new StringBuilder(
				"common/delaySpinner, lib/jquery.dataTables.min, shared/copyToStardev, details/shared, lib/prettify, details/benchmark");
		...
		// Set the page title to be the name of the benchmark if we're showing the
		// benchmark name.
	...
	private static void sendErrorMessage(int benchId, HttpServletResponse response) throws IOException {
		if (Benchmarks.isBenchmarkDeleted(benchId)) {
			...
			response.sendError(HttpServletResponse.SC_NOT_FOUND,
					"This benchmark has been moved to the trash bin by its owner.");
		} else {
