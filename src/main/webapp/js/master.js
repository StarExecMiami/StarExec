...
$(document).ready(function () {
  ...
  $navItems.find("ul.subnav a").on("keydown", function (e) {
    ...
    var $parentTrigger = $(this)
      .closest("li")
      .parent()
      .siblings("a[aria-haspopup]");
...
jQuery(function ($) {
  ...
  $(document).on("draw.dt", function (e, settings) {
    ...
    $(settings.oInstance) // The DataTable being drawn
      .parents("fieldset")
      .find(".list-count") // Find the label
...
/**
 * Function to display a message to the user using the centralized Alerts utility.
 * This function maintains backwards compatibility with the original API while
 * delegating to the modern Alerts system for better semantics and styling.
 *
 * CENTRALIZED ALERTING: This function now uses window.Alerts internally.
 * All 232+ existing callers continue to work without any changes.
 *
 * @param type The type of the message:
 *             - "error" or "err" → red alert
 *             - "warn" or "warning" → orange alert
 *             - "info" → blue alert
 *             - "success" → green alert
 * @param message The message to display (can contain HTML)
 * @param duration How long (in milliseconds) before auto-closing.
 *                 Anything <= 0 disables auto-hide.
 */
function showMessage(type, message, duration) {
  // Normalize type names for backwards compatibility
  ...
  // Validate that Alerts utility is available
  if (typeof window.Alerts === "undefined") {
    log("Alerts utility not loaded, falling back to legacy message system");
    showMessageLegacy(type, message, duration);
    return;
  }
  ...
  // Create alert using centralized utility
  var alertElement = Alerts.create({
    id: alertId,
    type: normalizedType,
    message: message,
    dismissible: true,
  ...
  // Insert into page (prepend to body, matching original behavior)
  Alerts.insert(alertElement, "body", "prepend");

  // Setup auto-dismiss timer (matching original behavior)
  if (duration > 0) {
    var timerId = setTimeout(function () {
      // Use Alerts utility for consistent dismissal with animation
      Alerts.dismiss(alertId, true);
    }, duration);

    // Store timer ID for potential cancellation
    $(alertElement).data("timer", timerId);
  }
}

/**
 * INTERNAL: Legacy message implementation (fallback only).
 * DO NOT CALL DIRECTLY - use showMessage() instead.
 * This is kept as fallback in case Alerts utility fails to load.
 *
 * @private
 */
function showMessageLegacy(type, message, duration) {
  ...
  var tmp =
    "<div><img src='" + starexecRoot + "images/icons/exclaim.png' /></div>";
  log(starexecRoot);
  ...
  // Hide the message, then put it at the top of the page and slide it down
  $(msg)
    .hide()
    .prependTo($("body"))
...
function addValidators() {
  $.validator.addMethod("regex", function (value, element, str) {
    var re = new RegExp(str);
    return this.optional(element) || re.test(value);
  });
  $.validator.addMethod("regexIgnoreCase", function (value, element, str) {
    var re = new RegExp(str, "i");
    return this.optional(element) || re.test(value);
  });
  $.validator.addMethod("jspregex", function (value, element, str) {
    return !element.validity.patternMismatch;
  });
  $.validator.addMethod("interval", function (value, element, str) {
