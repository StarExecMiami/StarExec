...
<%
	try {
		...
		request.setAttribute(
				"sites", Websites.getAllForHTML(id, WebsiteType.SPACE));
		...
		if (com == null) {
			...
			String url = Util.docRoot(
					"secure/edit/defaultPrimitive.jsp?id=" + settings.getId());

			request.setAttribute("defaultBenchLink", url + "&type=benchmark");
			request.setAttribute("defaultSolverLink", url + "&type=solver");
			request.setAttribute("com", com);
			request.setAttribute(
					"bench_proc", Processors
							.getByCommunity(id, ProcessorType.BENCH));
			request.setAttribute(
					"pre_proc", Processors
							.getByCommunity(id, ProcessorType.PRE));
			request.setAttribute(
					"post_proc", Processors
							.getByCommunity(id, ProcessorType.POST));
			request.setAttribute(
					"update_proc", Processors
							.getByCommunity(id, ProcessorType.UPDATE));
			request.setAttribute("defaultCpuTimeout", settings.getCpuTimeout());
			request.setAttribute(
					"defaultClockTimeout", settings.getWallclockTimeout());
			request.setAttribute(
					"usingBenchExec", settings.getBenchmarkingFramework() ==
							BenchmarkingFramework.BENCHEXEC);


			request.setAttribute(
					"defaultMaxMem",
					Util.bytesToGigabytes(settings.getMaxMemory())
			);
			request.setAttribute("settingId", settings.getId());
			try {
				List<Benchmark> benches =
						Benchmarks.get(settings.getBenchIds());
...
<star:template title="edit ${fn:escapeXml(com.name)}"
	...
	<fieldset id="websiteField">
		...
		<div id="newWebsite">
			<label for="website_name">name </label><input type="text"
			                                              id="website_name"/>
	...
	<fieldset id="benchmarkField">
		...
		<table id="benchTypeTbl" class="shaded">
			...
			<c:forEach var="proc" items="${bench_proc}">
				<tr id="proc_${proc.id}">
					<td>
						<a href="${starexecRoot}/secure/edit/processor.jsp?id=${proc.id}">${proc.name}
							<img class="extLink"
		...
		<form id="newTypeForm" class="newForm" enctype="multipart/form-data"
			...
			<table id="newTypeTbl">
				...
				<tr>
					...
					<td>
						<div class="processorLocalDiv">
							...
							<input type="radio" id="typeRadioLocal"
							       class="radioLocal" name="uploadMethod"
							       value="local" checked="checked"/>
						</div>
						<br>
						<div class="processorUrlDiv">
							<label>URL</label>
							<input type="radio" id="typeRadioURL"
							       class="radioURL" name="uploadMethod"
							       value="URL"/>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<span class="processorFileSpan"><input name="file"
						                                       type="file"
						                                       class="processorFile"/></span>
	...
	<fieldset id="settingsField">
		...
		<table id="settings" class="shaded">
			...
			<tr>
				...
				<td>
					<select id="editBenchmarkingFramework"
					        name="editBenchmarkingFramework">
						<c:if test="${!usingBenchExec}">
							<option value="RUNSOLVER" selected="selected">
								runsolver
							</option>
						...
						<c:if test="${usingBenchExec}">
							...
							<option value="BENCHEXEC" selected="selected">
								BenchExec
							</option>
						</c:if>
					</select>
			...
			<c:forEach items="${defaultBenchmarks}" var="defaultBench">
				<tr id="defaultBenchRow">
					...
					<td>${defaultBench.getName()}<span
							class="selectPrim deleteBenchmark"
							value="${defaultBench.id}">clear benchmark</span>
					</td>
				</tr>
			</c:forEach>
			<tr id="addDefaultBenchmark">
				<td>new default benchmark</td>
				<td><a href="${defaultBenchLink}"><span class="selectPrim">add benchmark</span></a>
				</td>
			</tr>
			<tr id="defaultSolverRow">
				<td>default solver</td>
				<td>${defaultSolver} <a href="${defaultSolverLink}"><span
						class="selectPrim">select solver</span></a></td>
			</tr>
			</tbody>
		</table>
	</fieldset>
		...
		<form id="addPreProcessorForm" class="newForm"
			...
			<table id="newPreProcessTbl">
				...
				<tr>
					...
					<td><textarea name="desc" id="processorDesc"></textarea>
					</td>
				</tr>
				<tr>
					<td>upload method</td>
					<td>
						<div class="processorLocalDiv">
							<label>local file</label>
							<input type="radio" id="preRadioLocal"
							       class="radioLocal" name="uploadMethod"
							       value="local" checked="checked"/>
						</div>
						<br>
						<div class="processorUrlDiv">
							<label>URL</label>
							<input type="radio" id="preRadioURL"
							       class="radioURL" name="uploadMethod"
							       value="URL"/>
						</div>
					</td>
	...
	<fieldset id="postProcessorField">
		...
		<table id="postProcessorTbl" class="shaded">
			...
			<c:forEach var="proc" items="${post_proc}">
				<tr id="proc_${proc.id}">
					<td>
						<a href="${starexecRoot}/secure/edit/processor.jsp?id=${proc.id}">${proc.name}<img
								class="extLink"
			...
			<table id="newPostProcessTbl">
				...
				<tr>
					...
					<td>
						<div class="processorLocalDiv">
							<label>local file</label>
							<input type="radio" id="postRadioLocal"
							       class="radioLocal" name="uploadMethod"
							       value="local" checked="checked"/>
						</div>
						<br>
						<div class="processorUrlDiv">
							<label>URL</label>
							<input type="radio" id="postRadioURL"
							       class="radioURL" name="uploadMethod"
							       value="URL"/>
						</div>
					</td>
			...
			<table id="newUpdateProcessTbl">
				...
				<tr>
					...
					<td>
						<div class="processorLocalDiv">
							<label>local file</label>
							<input type="radio" id="updateRadioLocal"
							       class="radioLocal" name="uploadMethod"
							       value="local" checked="checked"/>
						</div>
						<br>
						<div class="processorUrlDiv">
							<label>URL</label>
							<input type="radio" id="updateRadioURL"
							       class="radioURL" name="uploadMethod"
							       value="URL"/>
						</div>
					</td>
