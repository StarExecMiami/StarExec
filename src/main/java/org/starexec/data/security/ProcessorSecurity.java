...
public class ProcessorSecurity {
	...
	/**
	 * Checks to see whether the given user is the owner of a processor, meaning they can edit / delete it
	 *
	 ...
	 * @param userId The ID of the user making the request
	 * @return new ValidatorStatusCode(true) if the operation is allowed and a status code from ValidatorStatusCodes
	 * otherwise If the user lacks the necessary permissions for even one solver, a status code will be returned
	 */
	...
	public static ValidatorStatusCode canUserSeeProcessor(int procId, int userId) {
		...
		if (!GeneralSecurity.hasAdminReadPrivileges(userId) && procId != R.NO_TYPE_PROC_ID &&
				!Users.isMemberOfCommunity(userId, p.getCommunityId())) {
