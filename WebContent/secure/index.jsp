...
<%
	...
	System.out.println("unauthorized = " +
			                   user.getRole().equals(R.UNAUTHORIZED_ROLE_NAME));
	System.out.println(
			"suspended = " + user.getRole().equals(R.SUSPENDED_ROLE_NAME));
	...
	request.setAttribute(
			"isUnauthorized", user.getRole().equals(R.UNAUTHORIZED_ROLE_NAME));
	request.setAttribute("isSuspended",
	                     user.getRole().equals(R.SUSPENDED_ROLE_NAME)
	);
...
<star:template title="Starexec Preview">
	<c:if test="${isUnauthorized}">
		...
		<p>Once a leader of that community has approved your request, you
			...
			service. You may login as a guest user to explore our public
			offerings.</p>
		<br/>
	...
	<c:if test="${isSuspended}">
		<p>
			<strong>You have been suspended and have indefinitely lost access to
				StarExec services.</strong>
		</p>
		<br/>
		<p>You will be automatically logged out in 20 seconds.</p>
		<script>
