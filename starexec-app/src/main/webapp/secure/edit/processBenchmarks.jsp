<%@page contentType="text/html" pageEncoding="UTF-8"
        import="org.starexec.data.database.Processors, org.starexec.data.database.Spaces, org.starexec.data.database.Users,org.starexec.data.security.GeneralSecurity, org.starexec.data.to.Processor, org.starexec.data.to.enums.ProcessorType, org.starexec.util.SessionUtil, java.util.List"
        session="true" %>
...
<%
	try {
		...
		if (Users.isMemberOfSpace(userId, spaceId) ||
			...
			List<Processor> procs = Processors
					.getByCommunity(Spaces.getCommunityOfSpace(spaceId),
					                ProcessorType.BENCH
					);
	...
	<form id="processBenchForm" method="post"
		...
		<fieldset>
			...
			<table id="processorSelectionTable" class="shaded">
				...
				<c:forEach var="proc" items="${procs}">
					<tr>
						<td><input name="pid" type="hidden" value="${proc.id}"/><a
								href="${starexecRoot}/secure/edit/processor.jsp?id=${proc.id}">${proc.name}
							<img class="extLink"
		...
		<fieldset>
			...
			<table id="detailsTbl" class="shaded">
				...
				<tbody>
				<tr title="Decide whether to process all the benchmarks in the hierarchy rooted at this space or process only this space"
				    class="noHover">
					<td class="label">process hierarchy</td>
					<td>
						<div class="mobileBlock">
							<input type="radio" name="hier" id="hierTrue"
							       value="true"/>full hierarchy
						</div>
						<div class="mobileBlock">
							<input type="radio" name="hier" id="hierFalse"
							       checked="checked" value="false"/>this space
						</div>
					</td>
				</tr>
		 ...
		 a new attribute has the same name as an old one" class="noHover">
					...
					<td>
						<div class="mobileBlock">
							<input type="radio" name="clear" id="clearOldTrue"
							       value="true"/>yes
						</div>
						<div class="mobileBlock">
							<input type="radio" name="clear" id="clearOldFalse"
							       checked="checked" value="false"/>no
						</div>
					</td>
				</tr>
