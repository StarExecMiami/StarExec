...
2. **Base Helm values** (`values.yaml`) - Base defaults
3. **Environment Helm values** (`values-ENV.yaml`) - Deployment-specific
...
```bash
# Preferred: point at a mounted secret file (takes precedence when set)
export STAREXEC_DB_PASSWORD_FILE=/run/secrets/starexec-db-password

# Alternative: inline value from a secret manager
...
**Security Note:** Use `STAREXEC_DB_PASSWORD_FILE` in production to avoid exposing passwords in process listings. The Makefile and `docker-compose.yml` default the password to `starexec_dev_password` for development; production deployments must override it.
...
| Variable | Default | Required | Example | Notes |
|----------|---------|----------|---------|-------|
| `STAREXEC_BACKEND_TYPE` | `local` | No | `podman` | Backend: local, podman, kubernetes, k8s, kubernetes-native, sge, oar (legacy). Note: Makefile `make start` uses Podman deployment by default. |
...
├── values-kubernetes.yaml      # Kubernetes-native backend profile
├── values-microk8s.yaml        # Single-node MicroK8s profile
├── values-podman.yaml          # Podman deployment profile
├── values-podman.yaml.example  # Template for Podman overrides
├── values-auto-detected.yaml   # Generated by `make k8s-generate-values`
...
```yaml
# values-local-dev.yaml (abridged)
image:
...
postgres:
  host: localhost          # In-pod embedded PostgreSQL
  port: 5432
  user: starexec
  database: starexec
  password: "starexec_dev_password"      # Dev only!
  rootPassword: "starexec_dev_password"  # Dev only!
