 ...
 * Implements autocloseable so it can be used in a try-with-resources statement
 * and automatically
 ...
 *
 */
public class ResourceLoader implements AutoCloseable, IResourceLoader {
	private final StarLogger log = StarLogger.getLogger(ResourceLoader.class);

	// this class keeps track of all the primitives it creates. Calling
	// deleteAllPrimitives
	...
	/**
	 * Deletes all of the primitives that were created using any of the 'load'
	 * methods
	 * of this class
	 */
	...
	/**
	 * Returns the path to the resource directory, which is where we are storing
	 * files like solvers
	...
	/**
	 * Returns a File object representing the file with the given name from the
	 * resource directory
	 * 
	 * @param name The name of the file, which must be present in the resource
	 *             directory
	 * @return
	 */
	...
	/**
	 * Returns a File object representing the directory where all downloads
	 * performed during testing
	 * should be placed. For example, StarexecCommand tests use this downloads
	 * directory.
	 * 
	 * @return
	 */
	...
	public Job loadJobIntoDatabase(int spaceId, int userId, int preProcessorId, int postProcessorId, int solverId,
			List<Integer> benchmarkIds,
			int cpuTimeout, int wallclockTimeout, int memory) {
		List<Integer> solvers = new ArrayList<>();
		solvers.add(solverId);
		return loadJobIntoDatabase(spaceId, userId, preProcessorId, postProcessorId, solvers, benchmarkIds, cpuTimeout,
				wallclockTimeout, memory);
	}
	 ...
	 * a single configuration will be picked randomly to be added to the job, and
	 * the job will be added to a random
	 * queue that the given user owns. The job pairs will all be set to status
	 * 'complete' and given some fake output,
	 ...
	 * @param solverIds        The solvers to use for the job, which need to have at
	 *                         least 1 configuration each. The first
	 *                         configuration for every solver will be matched with
	 *                         every benchmark given
	 ...
	 * @param memory           The max memory limit, in bytes, for every pair in
	 *                         this job
	 * @return The job object
	 */
	public Job loadJobIntoDatabase(int spaceId, int userId, int preProcessorId, int postProcessorId,
			List<Integer> solverIds, List<Integer> benchmarkIds,
			int cpuTimeout, int wallclockTimeout, int memory) {
	...
	public DefaultSettings loadDefaultSettingsProfileIntoDatabaseWithDefaultBenchmarks(int userId,
			List<Integer> benchIds) throws SQLException {
	...
	public Configuration loadConfigurationIntoDatabase(String contents, int solverId) {
		try {
			...
			// Build a path to the appropriate solver bin directory and ensure the file
			// pointed to by newConfigFile doesn't already exist
			File newConfigFile = new File(Util.getSolverConfigPath(solver.getPath(), c.getName()));
			// If a configuration file exists on disk with the same name, just throw an
			// error. This really should never
			// happen because we are given the configs long, random names, so we have a
	...
	/**
	 * Loads an archive of benchmarks into the database. All benchmarks will be
	 * given a single random,
	 * unique attribute after they have been added. They will also be given
	 * randomized names that
	 * tests can assume are unique.
	 * 
	 * @param archiveName   The name of the archive containing the benchmarks in the
	 *                      Resource directory
	 * @param parentSpaceId The ID of the space to place the benchmarks in.
	 *                      Benchmarks will
	 *                      not be made into a hierarchy-- they will all be placed
	...
	public List<Integer> loadBenchmarksIntoDatabase(String archiveName, int parentSpaceId, int userId) {
		try {
			...
			// make a copy of the archive, because the benchmark extraction function will
			// delete the archive
			...
			List<Integer> ids = UploadBenchmark.addBenchmarksFromArchive(archiveCopy, userId, parentSpaceId,
					Processors.getNoTypeProcessor().getId(), false, p,
					"dump", statusId, false, false, null);
			for (Integer i : ids) {
				Benchmarks.updateDetails(i, TestUtil.getRandomAlphaString(DB.BENCH_NAME_LEN - 2),
						TestUtil.getRandomAlphaString(50),
	 ...
	 * 
	 * @param archiveName   The name of the archive containing the solver in the
	 *                      Resource directory
	...
	/**
	 * Creates a new SolverPipeline for the given user, where a stage is created for
	 * each given
	 * configuration. The stages will always depend on previous stages and also on
	 * another fake input,
	...
	/**
	 * Loads a user into the database, without any particular name, email, password,
	 * and so on. Useful for testing.
	 * 
	...
	public User loadUserIntoDatabase(String password, String role) {
		return loadUserIntoDatabase(TestUtil.getRandomAlphaString(10), TestUtil.getRandomAlphaString(10), password,
				password, "The University of Miami", role);
	}
	 ...
	 * @return The User on success, or null on error. Their ID will be set on
	 *         success.
	 */
	public User loadUserIntoDatabase(String fname, String lname, String email, String password, String institution,
			String role) {
		User u = new User();
	...
	/**
	 * Returns a WebDriver for selenium testing. The driver we be logged into the
	 * website
	 * upon return
	 * 
	...
	/**
	 * Writes 1000 characters of output to the location this pairs output should be
	 * placed
	 * 
	 * @param pair
	 */
	 ...
	 * 
	 * @return A size-3 array containing the absolute path to the config directory
	 *         and the two config names, in that order.
	 * @throws IOException
	 */
