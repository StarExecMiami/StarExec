...
public class KubernetesBackend implements Backend {
    ...
    private int generateExecId() throws Exception {
        if (activeIds.size() == Integer.MAX_VALUE) {
            throw new Exception(
                "Cannot support more that Integer.MAX_VALUE pairs"
            );
        }
        while (true) {
        ...
        try {
            ...
            Thread jobThread = new Thread(() -> {
                try {
                    ...
                    log.error(
                        "Error running job " + j.execId + ": " + e.getMessage(),
                        e
                    );
                }
            });
            ...
            log.error(
                "Error starting job " + j.execId + ": " + e.getMessage(),
                e
            );
        }
    }
    ...
    public synchronized void removeInactiveJobs() {
        Iterator<Map.Entry<Integer, LocalJob>> iterator = activeIds
            .entrySet()
            .iterator();
    ...
    public String[] getWorkerNodes() {
        String[] cmd = {
            ...
            "nodes",
            "-l",
            "nodegroup=computenodes", // <=== Filter here
            "-o",
    ...
    public Map<String, String> getNodeQueueAssociations() {
        ...
        try {
            ...
            log.info(
                "IOException in getNodeQueueAssociations: " + e.getMessage()
            );
    ...
    public boolean clearNodeErrorStates() {
        log.info(
            "clearNodeErrorStates() called, but not implemented. Doing nothing."
        );
        return true;
    }
    ...
    public void deleteQueue(String queueName) {
        ...
        String labelSelector =
            "nodegroup=computenodes," + QUEUE_LABEL_KEY + "=" + queueName;
        String[] getCmd = {
            "kubectl",
            "get",
            "nodes",
        ...
        try {
            ...
            log.info(
                "IOException in deleteQueue for '" +
                    queueName +
                    "': " +
                    e.getMessage()
            );
    ...
    public boolean createQueue(
        ...
        try {
            ...
            log.info(
                "IOException in createQueue for '" +
                    newQueueName +
                    "': " +
                    e.getMessage()
            );
    ...
    public boolean createQueueWithSlots(
        ...
        if (DEFAULT_QUEUE_NAME.equals(newQueueName)) {
            log.info(
                "createQueueWithSlots called for default queue. Nothing to do."
            );
            return true;
        }
        ...
        if (slots != null) {
            try {
                ...
                log.info(
                    "IOException in createQueueWithSlots for '" +
                        newQueueName +
                        "': " +
                        e.getMessage()
                );
    ...
    public void clearPairTracking(int pairId) {
        // KubernetesBackend does not track pair state - no-op
    }
    ...
    public void initialize(String BACKEND_ROOT) {
        final Runnable runLocalJobsRunnable = new RobustRunnable(
            "runLocalJobsRunnable"
        ) {
            @Override
