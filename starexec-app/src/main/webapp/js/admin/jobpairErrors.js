...
jQuery(function($) {
	 ...
	 */
	const CONFIG = {
		API_ENDPOINT: starexecRoot + "services/jobpairErrors",
		TIMEOUT: 10000,
		MAX_RETRIES: 3,
		RETRY_DELAY: 1000, // ms
	...
	const resultsTable = $("#jobpairErrors").DataTable(resultsTableConfig);
	...
	const elements = {
		...
		$errorContainer: $("#jobpairerrors_error"),
		$infoContainer: $("#jobpairerrors_info"),
		$loadingSpinner: $("#jobpairerrors_loading"),
	};
	...
	const showError = (message) => {
		...
		console.error("[JobPairErrors Error]", message);
	};
	...
	const announceToScreenReader = (message) => {
		const $liveRegion = $("#jobpairerrors_live_region");
	...
	/**
	 * Formats jobpair data for table display
	 * @param {Object} jobPair - JobPair object with id
	 * @returns {string} HTML link to jobpair details
	 */
	const formatJobpair = (jobPair) => {
		if (!jobPair || !jobPair.id) return "Unknown";
		const url = `${starexecRoot}secure/details/pair.jsp?id=${jobPair.id}`;
		return star.format.link(url, jobPair.id);
	};

	/**
	 * Populates DataTable with jobpair error results
	 * @param {Array} data - Array of jobpair error objects
	 */
	const populateTable = (data) => {
		if (!Array.isArray(data) || data.length === 0) {
			resultsTable.clear().draw();
			showInfo("No job pair errors found for selected dates.");
			announceToScreenReader("No errors available.");
			return;
		}

		resultsTable.clear();
		const rows = data.map((item) => [
			escapeHtml((item.node && item.node.name) || "Unknown"),
			formatJobpair(item.jobPair),
			star.format.timestamp(item.time),
		]);
		resultsTable.rows.add(rows).draw();

		const message = `Loaded ${data.length} error(s).`;
	...
	const getErrorMessage = (status) => {
		const messages = {
			...
			403: "You do not have permission to view job pair errors.",
			404: "Job pair errors service not found.",
			500: "Server error. Please try again later.",
			504: "Service timeout. Please try again.",
		};
		return messages[status] || "Failed to load job pair errors. Please try again.";
	};

	/**
	 * Fetches jobpair error data from API with retry logic
	 * @param {number} retryCount - Current retry attempt (default: 0)
	 */
	const fetchErrors = (retryCount = 0) => {
		...
		$.ajax({
			...
			error: (jqXHR, textStatus, errorThrown) => {
				...
				if (
					...
					console.warn(
						`[JobPairErrors] Retry ${retryCount + 1}/${
							CONFIG.MAX_RETRIES
						} after ${delay}ms`
					);
					setTimeout(() => {
						fetchErrors(retryCount + 1);
					}, delay);
					return;
				}
	...
	const setDatePreset = (presetKey) => {
		...
		if (!preset) {
			console.warn(`[JobPairErrors] Unknown preset: ${presetKey}`);
			return;
		}
	...
	const init = () => {
		...
		$(document).on("click", "[data-jobpairerrors-preset]", (e) => {
			const presetKey = $(e.target).data("jobpairerrors-preset");
