<%@page import="org.starexec.constants.R, org.starexec.data.to.enums.Primitive" %>
<%@page import="org.starexec.util.JspHelpers" %>
...
<star:template title="${benchPageTitle}" js="${js}"
               css="common/delaySpinner, details/shared, common/table, details/benchmark, shared/copyToStardev, prettify/prettify">
	<star:primitiveTypes/>
	...
	<c:if test="${!isAnonymousPage}">
		...
		<star:primitiveIdentifier primId="${bench.id}"
		                          primType="${primitiveType.toString()}"/>
		<span style="visibility: hidden" class="benchProcessorId"
		      value="${bench.type.id}"></span>
	</c:if>
	<c:if test="${!isAnonymousPage}">
		...
		<fieldset id="fieldDepends">
			...
			<table class="shaded">
				...
				<c:forEach var="dependency" items="${depends}">
					<tr>
						<td><star:benchmark
								value="${dependency.secondaryBench}"/></td>
	...
	<c:if test="${downloadable}">
		<fieldset id="fieldContents">
			...
			<a href="${starexecRoot}/services/benchmarks/${bench.id}/contents?limit=-1"
			   target="_blank" class="popoutLink">popout</a>
			<p class="caption">content may be truncated. 'popout' for larger
				text window.</p>
		</fieldset>
	</c:if>
	...
	<fieldset id="actions">
		...
		<c:if test="${downloadable || hasAdminReadPrivileges}">
			<a id="downLink"
			   href="${starexecRoot}/secure/download?type=bench&id=${bench.id}">download
				benchmark</a>
		</c:if>
