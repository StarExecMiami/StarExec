<%@page contentType="text/html" pageEncoding="UTF-8"
        import="org.starexec.data.to.enums.Primitive,org.starexec.util.JspHelpers" %>
...
<star:template title="${solverPageTitle}"
	...
	<c:if test="${ !isAnonymousPage }">
		...
		<star:primitiveIdentifier primId="${solver.id}"
		...
		<fieldset id="fieldSites">
			...
			<table class="shaded">
				...
				<c:forEach var="site" items="${sites}">
					<tr>
						<td>${site[0]}</td>
						<td><a href="${site[1]}">${site[2]}</a><img
								class="extLink"
								src="${starexecRoot}/images/external.png"/></td>
	...
	<c:if test="${!sourceDownloadable}">
		<div id="dialog-building-job" title="Building..." class="hiddenDialog">
			<p><span class="ui-icon ui-icon-info" aria-hidden="true"></span><span
					id="dialog-building-job-txt"></span></p>
		</div>
	</c:if>
