...
function initUI() {
  ...
  $("#saveProfile").click(function () {
    ...
    $.post(
      ...
      {
        postp: $("#editPostProcess").val(),
        prep: $("#editPreProcess").val(),
        benchp: $("#editBenchProcess").val(),
        solver: $("#solver").val(),
        name: $("#settingName").val(),
        cpu: $("#cpuTimeout").val(),
        wall: $("#wallclockTimeout").val(),
        dep: $("#editDependenciesEnabled").val(),
        "bench[]": benchmarkIds,
        mem: $("#maxMem").val(),
        settingId: $("#settingProfile").val(),
        userIdOfOwner: userId,
        benchmarkingFramework: getNewBenchmarkingFramework(),
      },
      function (returnCode) {
        ...
        create: function () {
          ...
          $.post(
            ...
            if (xhr.statusText === "Forbidden") {
              showMessage(
                "error",
                "You do not have permission to add a new profile for this user.",
                5000
              );
            } else {
              showMessage("error", "Invalid parameters", 5000);
  ...
  $("#deleteUser").click(function (e) {
    $("#dialog-confirm-delete-txt").text(
      "Deleting a user will delete all their primitives.\nAre you sure you want to delete this user?"
    );
...
function editable(attribute) {
  $("#edit" + attribute).click(function () {
    ...
    $("#save" + attribute).click(function () {
      saveChanges(this, true, attribute, old);
    });
